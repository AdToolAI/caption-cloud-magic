...
export default function LocationLightingGrid({
  ...
  const handleSetPrimary = useCallback(
    async (variant: LocationVariant) => {
      ...
      if (ok) {
        toast.success(tx({ de: "Primäre Lichtstimmung gesetzt", en: "Primary lighting set", es: "Iluminación principal establecida" }));
        await reload();
    ...
    <div className="space-y-3">
      <div className="space-y-0.5">
        <p className="text-xs font-semibold tracking-wide text-foreground/90">
          {tx({ de: "Lichtstimmungen · Location-Scouting", en: "Lighting moods · location scouting", es: "Ambientes de luz · scouting de localizaciones" })}
        </p>
