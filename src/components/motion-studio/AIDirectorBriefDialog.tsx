...
export default function AIDirectorBriefDialog({
  ...
  const apply = () => {
    ...
    setBrief('');
    setPlan(null);
    toast.success(tx({ de: `Storyboard "${plan.title}" geladen — ${plan.scenes.length} Szenen ✨`, en: `Storyboard "${plan.title}" loaded — ${plan.scenes.length} Scenes ✨`, es: `Guión gráfico "${plan.title}" cargado — ${plan.scenes.length} Escenas ✨` }));
  };
