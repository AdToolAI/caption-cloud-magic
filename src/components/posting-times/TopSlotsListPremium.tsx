  ...
  return (
    <div className="relative w-12 h-12">
      <svg className="w-12 h-12 -rotate-90">
        <circle cx="24" cy="24" r="18" stroke="currentColor" strokeWidth="3" fill="none" className="text-muted/20" />
        <motion.circle
          initial={{ strokeDashoffset: circumference }}
          animate={{ strokeDashoffset }}
          transition={{ duration: 1, ease: "easeOut" }}
          cx="24" cy="24" r="18" stroke={getColor()} strokeWidth="3" fill="none" strokeLinecap="round"
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className={cn("text-lg font-bold", isToday && "text-primary")}>
                  {format(new Date(day.date), 'EEEE', { locale: dateFnsLocale })}
                </div>
                <div className="text-sm text-muted-foreground">
                  {format(new Date(day.date), 'd. MMMM', { locale: dateFnsLocale })}
                </div>
                {isToday && (
                  <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-primary/20 text-primary border border-primary/30">
                    {t('postingTimes.today')}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                <Sparkles className="w-3 h-3" />
                {t('postingTimes.topN')} {topSlots.length}
              </div>
            </div>
