      ...
      <div className="relative flex items-start justify-between">
        <div className="space-y-3">
          ...
          >
            ...
            <span className="text-xs font-medium text-cyan-400">{t('postingTimes.livePrognosis')}</span>
          </motion.div>
          ...
          {metadata && (
            <div className="text-right text-sm space-y-1">
              <div className="flex items-center gap-2 text-muted-foreground">
                <span>{t('postingTimes.updated')}:</span>
                <span className="font-mono text-foreground">
                  {format(new Date(metadata.generatedAt), 'HH:mm', { locale: dateFnsLocale })}
                </span>
              </div>
              {metadata.hasHistory && (
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Sparkles className="w-3 h-3 text-primary" />
                  <span>{metadata.historyDays} {t('postingTimes.daysHistory')}</span>
                </div>
              )}
            </div>
          )}
