  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast.success(tx({ de: 'Final Mix gespeichert', en: 'Final mix saved', es: 'Mezcla final guardada' }), {
        description: tx({ de: `Normalisiert auf ${mixer.target.label} (${mixer.target.lufs} LUFS)`, en: `Normalized to ${mixer.target.label} (${mixer.target.lufs} LUFS)`, es: `Normalizado a ${mixer.target.label} (${mixer.target.lufs} LUFS)` }),
      });
      onMixSaved?.();
    } catch (e: any) {
    ...
    <div className="space-y-4">
      ...
      <Card className="backdrop-blur-xl bg-gradient-to-br from-emerald-500/10 via-card/60 to-primary/10 border-emerald-500/30 p-5">
        ...
        <p className="text-[11px] text-muted-foreground text-center mt-2">
          {tx({ de: "Kostenlos · Loudness-Normalisierung mit Soft-Limiter (Clip-Schutz)", en: "Free · Loudness normalization with soft limiter (clip protection)", es: "Gratis · Normalización de sonoridad con limitador suave (protección contra recortes)" })}
        </p>
      </Card>
    </div>
  );
