    ...
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">
          PNG, JPG, WebP bis 10MB • Max. {maxFiles} {tx({ de: "Bilder", en: "Images", es: "Imágenes" })}
        </p>
      </div>
      ...
      {value.length > 0 && value.length < minFiles && (
        <p className="text-sm text-destructive">
          {tx({ de: "Noch", en: "Still", es: "Todavía" })} {minFiles - value.length} {tx({ de: "Bild(er) erforderlich", en: "Image(s) required", es: "Imagen(es) requerida(s)" })}
        </p>
      )}
    </div>
  );
