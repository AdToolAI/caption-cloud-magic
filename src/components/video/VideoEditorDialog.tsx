...
export const VideoEditorDialog = ({ open, onOpenChange, video }: VideoEditorDialogProps) => {
  ...
  const handleSave = async () => {
    ...
    if (result) {
      toast({ title: tx({ de: "Video wird generiert", en: "Video is generated", es: "Se genera vídeo" }), description: tx({ de: `Version ${result.version_number} wird erstellt.`, en: `Version ${result.version_number} is being created.`, es: `Se está creando la versión ${result.version_number}.` }) });
      onOpenChange(false);
    }
  };
