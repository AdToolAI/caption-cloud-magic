  ...
  const handleMultiImageUpload = async (key: string, images: Array<{ id: string; url: string; file: File }>) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        toast.error(tx({ de: tx({ de: `Bild-Upload fehlgeschlagen: ${error.message}`, en: `Image upload failed: ${error.message}`, es: `Error al subir la imagen: ${error.message}` }), en: `Image upload failed: ${error.message}`, es: `Error al subir la imagen: ${error.message}` }));
      } else {
      ...
      <DialogContent className="max-w-6xl max-h-[90vh]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {step === 'customize' && tx({ de: tx({ de: `Video erstellen: ${selectedTemplate?.name}`, en: `Create video: ${selectedTemplate?.name}`, es: `Crear vídeo: ${selectedTemplate?.name}` }), en: `Create video: ${selectedTemplate?.name}`, es: `Crear vídeo: ${selectedTemplate?.name}` })}
