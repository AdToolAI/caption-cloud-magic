...
export function ProductInsightBanner({ suggestion, onApply, applied = false }: ProductInsightBannerProps) {
  ...
  const categoryLabels: Record<string, string> = useMemo(() => ({
    workspace: t('picStudio.catWorkspace'),
    outdoor: t('picStudio.catOutdoor'),
    urban: t('picStudio.catUrban'),
    studio: t('picStudio.catStudio'),
    wellness: t('picStudio.catWellness'),
    tech: t('picStudio.catTech'),
    luxury: t('picStudio.catLuxury'),
  }), [t]);
      ...
      <div className="relative backdrop-blur-xl bg-card/60 border border-cyan-500/20 rounded-xl p-5">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h4 className="text-sm font-bold">{t('picStudio.aiProductRecognition')}</h4>
              <Badge variant="outline" className="text-[10px] px-1.5 py-0 border-cyan-500/30 text-cyan-400">
                v3
              </Badge>
            </div>
            <p className="text-xs text-muted-foreground">
              {t('picStudio.recognized')} <span className="text-foreground font-medium">{suggestion.productType}</span>
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-3">
          <div className="flex items-center gap-2 p-2 rounded-lg bg-muted/20 border border-white/5">
            ...
            <div>
              <p className="text-[10px] text-muted-foreground">{t('picStudio.insightCategory')}</p>
          ...
          <div className="flex items-center gap-2 p-2 rounded-lg bg-muted/20 border border-white/5">
            ...
            <div>
              <p className="text-[10px] text-muted-foreground">{t('picStudio.insightLight')}</p>
          ...
          <div className="flex items-center gap-2 p-2 rounded-lg bg-muted/20 border border-white/5">
            ...
            <div>
              <p className="text-[10px] text-muted-foreground">{t('picStudio.insightIntensity')}</p>
        ...
        >
          {applied ? (
            ...
            t('picStudio.applyRecommendation')
          )}
        </Button>
      </div>
    </motion.div>
