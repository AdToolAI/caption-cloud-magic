  ...
  const handleConnect = async (providerId: string, providerName: string) => {
    ...
    try {
      ...
      const oauthUrls: Record<string, string> = {
        ...
        tiktok: `https://www.tiktok.com/v2/auth/authorize?client_key=${import.meta.env.VITE_TIKTOK_CLIENT_KEY}&response_type=code&scope=user.info.basic,video.list,video.upload&redirect_uri=${encodeURIComponent(redirectUri + '?provider=tiktok')}&state=${encodeURIComponent(state)}`,
