...
const copy = {
  ...
  en: {
    ...
    personalizedTitle: "Tailored for you",
  ...
  es: {
    ...
    personalizedTitle: "Personalizado para ti",
    ...
    >
      ...
      <div className="relative p-5 md:p-6">
        <div className="flex flex-col md:flex-row md:items-start gap-5">
          ...
          <Link
            to={`/hailuo-video-studio?prompt=${encodeURIComponent(prompts[0]?.prompt ?? "")}&prompt_en=${encodeURIComponent(prompts[0]?.prompt_en ?? "")}`}
        ...
        <div className="mt-5 pt-5 border-t border-border/50">
          <p className="text-[11px] uppercase tracking-[0.15em] text-muted-foreground mb-3 font-medium">
            {personalized ? t.personalizedTitle : t.suggestionsTitle}
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
            {prompts.map((p, i) => (
              <Link
                key={i}
                to={`/hailuo-video-studio?prompt=${encodeURIComponent(p.prompt)}&prompt_en=${encodeURIComponent(p.prompt_en)}`}
