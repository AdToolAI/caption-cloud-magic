...
export function ToolkitGenerator({ onAfterGenerate }: Props) {
  ...
  const runGenerate = async () => {
    ...
    try {
      ...
      toast.success(
        language === 'de'
          ? tx({ de: tx({ de: `Video wird generiert (${model.name}). Kosten: ${symbol}${cost.toFixed(2)}`, en: `Video is generated (${model.name}). Cost: ${symbol}${cost.toFixed(2)}`, es: `Se genera el vídeo (${model.name}). Costo: ${symbol}${cost.toFixed(2)}` }), en: `Video is generated (${model.name}). Cost: ${symbol}${cost.toFixed(2)}`, es: `Se genera el vídeo (${model.name}). Costo: ${symbol}${cost.toFixed(2)}` })
                            ...
                            if (ch?.voice_id) {
                              return (
                                <p className="text-[10px] text-muted-foreground italic">
                                  {language === 'de'
                                    ? tx({ de: tx({ de: `Hinweis: Cast-Stimme hinterlegt — wird in Motion Studio / Voiceover verwendet. Kling Omni nutzt hier eigene Presets.`, en: `Note: Cast voice stored — will be used in Motion Studio / Voiceover. Kling Omni uses its own presets here.`, es: `Nota: Voz del elenco guardada — se usará en Motion Studio / Voiceover. Kling Omni usa sus propios preajustes aquí.` }), en: `Note: Cast voice stored — will be used in Motion Studio / Voiceover. Kling Omni uses its own presets here.`, es: `Nota: Voz del elenco guardada — se usará en Motion Studio / Voiceover. Kling Omni usa sus propios preajustes aquí.` })
