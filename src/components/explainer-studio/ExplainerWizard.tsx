...
import { ExplainerPreview } from './ExplainerPreview';
...
export function ExplainerWizard({ project, onProjectUpdate }: ExplainerWizardProps) {
  ...
  const [selectedExportFormats, setSelectedExportFormats] = useState<string[]>(['landscape']);  // ✅ Store selected formats
  ...
  const handleAutoGenerationComplete = (project: any) => {
    ...
    if (project.characterSheetUrl) setCharacterSheetUrl(project.characterSheetUrl);
    
    // Create briefing from consultation result for export step
    if (consultationResult && !briefing) {
      setBriefing({
        productDescription: consultationResult.productSummary || '',
        targetAudience: consultationResult.targetAudience || [],
        ...
        character: consultationResult.characterPreferences || { hasCharacter: false },
        voiceId: consultationResult.audioPreferences?.voiceId || 'EXAVITQu4vr4xnSDxMaL',
        voiceName: consultationResult.audioPreferences?.voiceName || 'Bella',
      });
    }
  ...
  const handleSwitchToManual = (partialProject: any) => {
    ...
    if (partialProject?.characterSheetUrl) setCharacterSheetUrl(partialProject.characterSheetUrl);
  ...
  // Regenerate a single scene with character consistency
  const handleRegenerateScene = useCallback(async (sceneId: string) => {
    if (!autoGeneratedProject?.script?.scenes) return;
    
    const scene = autoGeneratedProject.script.scenes.find((s: any) => s.id === sceneId);
    if (!scene) return;
    ...
    const { data, error } = await supabase.functions.invoke('generate-premium-visual', {
      body: {
        ...
        style: autoGeneratedProject.briefing?.style || consultationResult?.recommendedStyle || 'flat-design',
        character: autoGeneratedProject.briefing?.character || consultationResult?.characterPreferences,
        characterSheetUrl: characterSheetUrl || autoGeneratedProject.characterSheetUrl,
        styleGuide: consultationResult?.extractedStyleGuide,
      }
    });
    ...
    if (data?.imageUrl) {
      ...
      setAssets(prev => {
        ...
        const existingIndex = newAssets.findIndex(a => a.sceneId === sceneId);
        const newAsset = {
          id: crypto.randomUUID(),
          sceneId,
          type: 'background' as const,
          imageUrl: data.imageUrl,
          prompt: data.prompt || scene.visualDescription,
          style: autoGeneratedProject.briefing?.style || 'flat-design',
          isPremium: true,
        };
      ...
      // Update autoGeneratedProject assets as well
      setAutoGeneratedProject((prev: any) => ({
        ...prev,
        assets: prev.assets?.map((a: any) => 
          a.sceneId === sceneId 
            ? { ...a, imageUrl: data.imageUrl, prompt: data.prompt }
            : a
        ) || [],
      }));
    }
  }, [autoGeneratedProject, consultationResult, characterSheetUrl]);

  // ✅ Fixed: Handle formats from preview and trigger multi-format export
  const handlePreviewConfirm = async (formats: string[]) => {
    console.log('[ExplainerWizard] ✅ Multi-format export triggered with formats:', formats);
    setSelectedExportFormats(formats);
    
    // Trigger multi-format render immediately if multiple formats selected
    if (formats.length >= 1 && autoGeneratedProject) {
      try {
        toast.info(`🎬 Starte Export für ${formats.length} Format(e)...`);
        
        const { data, error } = await supabase.functions.invoke('render-explainer-video', {
          body: {
            scenes: autoGeneratedProject.script?.scenes || [],
            voiceoverUrl: autoGeneratedProject.voiceoverUrl,
            backgroundMusicUrl: autoGeneratedProject.backgroundMusicUrl,
            subtitles: autoGeneratedProject.subtitles || [],
            style: briefing?.style || 'flat-design',
            primaryColor: (consultationResult?.extractedStyleGuide as any)?.colorPalette?.primary || '#F5C76A',
            multiFormat: formats.length > 1,
            formats: formats,
            format: formats[0],  // Primary format
            projectId: autoGeneratedProject.id,
          }
        });
        
        if (error) {
          console.error('[ExplainerWizard] Multi-format render error:', error);
          toast.error('Export fehlgeschlagen: ' + error.message);
        } else {
          console.log('[ExplainerWizard] ✅ Multi-format render started:', data);
          toast.success(`🎬 Export für ${formats.length} Format(e) gestartet!`);
        }
      } catch (err) {
        console.error('[ExplainerWizard] Export error:', err);
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {/* Preview (Full-Service after generation) - Live Video Preview */}
          ...
          {/* Export (Full-Service after preview confirmation) */}
          {currentStepId === 'export' && generationMode === 'full-service' && autoGeneratedProject && briefing && script && (
            <div className="space-y-6">
              ...
              <ExportStep
                ...
                audioConfig={audioConfig || {
                  ...
                  backgroundMusicUrl: autoGeneratedProject.backgroundMusicUrl || null,
                  musicVolume: 0.3,
                  voiceVolume: 1,
                  language: 'de',
                  soundEffects: [],
          ...
          {currentStepId === 'export' && generationMode === 'manual' && briefing && script && assets.length > 0 && animationConfig && audioConfig && (
            <ExportStep
              briefing={briefing}
              script={script}
              assets={assets}
