    ...
    return (
      <Card className="p-8 text-center border-dashed bg-muted/20">
        ...
        <p className="text-sm text-muted-foreground max-w-md mx-auto">
          {tx({ de: <>{tx({ de: "Der Autopilot wertet erst aus, sobald mindestens", en: "The autopilot only analyzes once at least <strong>10 posts</strong> have gone live. Currently analyzed: {insights?.total_posts_analyzed ?? 0} posts.", es: "El piloto automático solo analiza una vez que al menos <strong>10 publicaciones</strong> hayan sido publicadas. Actualmente analizadas: {insights?.total_posts_analyzed ?? 0} publicaciones." })}<strong>10 Posts</strong> live waren. Aktuell analysiert: {insights?.total_posts_analyzed ?? 0} Posts.</>, en: <>The autopilot only analyzes once at least <strong>10 posts</strong> have gone live. Currently analyzed: {insights?.total_posts_analyzed ?? 0} posts.</>, es: <>El piloto automático solo analiza una vez que al menos <strong>10 publicaciones</strong> están activas. Analizado actualmente: {insights?.total_posts_analyzed ?? 0} publicaciones.</> })}
        </p>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {tx({ de: <>{tx({ de: "Analyse von", en: "Analysis of <strong>{insights.total_posts_analyzed}</strong> real posts · avg engagement <strong>{((insights.avg_engagement_rate ?? 0) * 100).toFixed(1)}%</strong>", es: "Análisis de <strong>{insights.total_posts_analyzed}</strong> publicaciones reales · engagement promedio <strong>{((insights.avg_engagement_rate ?? 0) * 100).toFixed(1)}%</strong>" })}<strong>{insights.total_posts_analyzed}</strong> echten Posts · ⌀ Engagement <strong>{((insights.avg_engagement_rate ?? 0) * 100).toFixed(1)}%</strong></>, en: <>Analysis of <strong>{insights.total_posts_analyzed}</strong> real posts · avg engagement <strong>{((insights.avg_engagement_rate ?? 0) * 100).toFixed(1)}%</strong></>, es: <>Análisis de <strong>{insights.total_posts_analyzed}</strong> publicaciones reales · engagement promedio <strong>{((insights.avg_engagement_rate ?? 0) * 100).toFixed(1)}%</strong></> })}
            </p>
