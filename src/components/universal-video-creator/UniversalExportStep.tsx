...
const EXPORT_FORMATS: ExportFormat[] = [
  { id: '16-9', aspectRatio: '16:9', label: 'YouTube / Website', description: '1920×1080', icon: Monitor, width: 1920, height: 1080 },
  { id: '9-16', aspectRatio: '9:16', label: 'TikTok / Reels', description: '1080×1920', icon: Smartphone, width: 1080, height: 1920 },
  { id: '1-1', aspectRatio: '1:1', label: 'Social Feed', description: '1080×1080', icon: Square, width: 1080, height: 1080 },
];
...
export function UniversalExportStep({ project, category, userId, onBack, onComplete }: UniversalExportStepProps) {
  ...
  const startRendering = async () => {
    if (selectedFormats.length === 0) {
      toast.error(t('uvc.exportSelectFormat'));
      return;
    }
    ...
    const initialStatuses: RenderStatus[] = selectedFormats.map(formatId => ({
      formatId, status: 'pending', progress: 0,
    }));
    ...
    for (const formatId of selectedFormats) {
      ...
      try {
        ...
        const response = await supabase.functions.invoke('render-universal-video', {
          body: { project, aspectRatio: format.aspectRatio, width: format.width, height: format.height, userId, category }
        });
        ...
        setRenderStatuses(prev => prev.map(s => 
          s.formatId === formatId ? { ...s, status: 'failed', error: err instanceof Error ? err.message : 'Rendering failed' } : s
        ));
      }
    }
  };
        ...
        if (done && outputFile) {
          ...
          setRenderStatuses(prev => {
            const updated = prev.map(s => 
              s.formatId === formatId ? { ...s, status: 'completed' as const, progress: 100, outputUrl: outputFile } : s
            );
            ...
            if (allDone) {
              ...
              if (completedRenders.length > 0) {
                toast.success(t('uvc.exportRenderedToast', { count: String(completedRenders.length) }));
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#F5C76A]/10 border border-[#F5C76A]/30 mb-4">
          ...
          <span className="text-sm font-medium text-[#F5C76A]">{categoryName} Export</span>
        </div>
        ...
        <p className="text-muted-foreground">{t('uvc.exportDesc')}</p>
      </motion.div>
      ...
      {!isRendering && !allCompleted && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4 mb-3">
                    <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center transition-all", isSelected ? "bg-[#F5C76A] text-black" : "bg-muted/20 text-muted-foreground")}>
                      <Icon className="h-6 w-6" />
                    </div>
                    <div className={cn("w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all", isSelected ? "bg-[#F5C76A] border-[#F5C76A]" : "border-muted-foreground")}>
                      {isSelected && <Check className="h-4 w-4 text-black" />}
                    </div>
                  </div>
                  <h3 className={cn("font-semibold mb-1", isSelected && "text-[#F5C76A]")}>{format.label}</h3>
                  <p className="text-sm text-muted-foreground">{format.description} • {format.aspectRatio}</p>
                </motion.button>
              );
            })}
          </div>
          ...
          <div className="flex items-center justify-between pt-4">
            {onBack && (
              <Button variant="outline" onClick={onBack}>{t('uvc.exportBackToPreview')}</Button>
            )}
            <Button
            ...
            >
              ...
              {t('uvc.exportRenderCount', { count: String(selectedFormats.length) })}
            </Button>
          </div>
        </motion.div>
      )}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <div className="bg-card/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm text-muted-foreground">{t('uvc.exportOverallProgress')}</span>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={cn(
                      ...
                      {status.status === 'completed' ? <Check className="h-5 w-5" /> :
                       status.status === 'failed' ? <AlertCircle className="h-5 w-5" /> :
                       status.status === 'rendering' ? <Loader2 className="h-5 w-5 animate-spin" /> :
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-sm text-muted-foreground">
                          {status.status === 'completed' ? t('uvc.exportCompleted') :
                           status.status === 'failed' ? t('uvc.exportFailed') :
                           status.status === 'rendering' ? `${Math.round(status.progress)}%` :
                           t('uvc.exportWaiting')}
                        </span>
                      </div>
                      {status.status === 'rendering' && <Progress value={status.progress} className="h-1.5" />}
                      {status.status === 'failed' && status.error && <p className="text-xs text-destructive mt-1">{status.error}</p>}
                    </div>
                    {status.status === 'completed' && status.outputUrl && (
                      <Button size="sm" onClick={() => downloadVideo(status.outputUrl!, status.formatId)} className="bg-[#F5C76A] text-black hover:bg-[#F5C76A]/90">
      ...
      {allCompleted && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8">
          ...
          <h3 className="text-2xl font-bold mb-2 text-green-500">{t('uvc.exportComplete')}</h3>
