...
export function UniversalVideoWizard() {
  ...
  /** Mount timestamp — hard lower bound for any DB recovery in this session. */
  const sessionStartedAtRef = useRef<string>(new Date().toISOString());
  /** True when the user explicitly started a new project (survives reload via localStorage flag). */
  const freshStartRef = useRef<boolean>(false);
  ...
  const handleResumeDraft = () => {
    ...
    if (draft) {
      ...
      // Resolve step by stepId if available, otherwise validate numeric step
      // Defer step resolution to after state is set (useEffect below will handle it)
      if (draft.stepId) {
        // Will be resolved after STEPS are computed with new state
        requestAnimationFrame(() => {
          // Re-read STEPS after state updates
  ...
  // Safe-step resolver: validate currentStep against actual state
  useEffect(() => {
    if (!draftChecked) return;
    ...
    // On preview/export without project -> recovery
    if ((stepId === 'preview' || stepId === 'export') && !autoGeneratedProject) {
      // Don't reset if DB fallback hasn't been attempted yet
      if (stepId === 'preview') return; // Let DB fallback effect handle it
      if (stepId === 'export') {
    ...
    // On consultation without category
    if (stepId === 'consultation' && !selectedCategory) {
      setCurrentStep(0);
      return;
    }
  }, [currentStep, draftChecked]);
  ...
  useEffect(() => {
    ...
    if (freshStartRef.current) return; // explicit new project — never recover
    if ((isStuckOnGenerating || isPreviewWithoutProject) && user && !dbFallbackAttempted.current) {
      ...
      import('@/integrations/supabase/client').then(({ supabase }) => {
        ...
        // Hard time bound: only results produced in (or after) the current
        // session may be recovered — never an old run after a reload.
        query = query.gte('created_at', generationStartedAtRef.current ?? sessionStartedAtRef.current);
  ...
  const handleConsultationSkip = () => {
    ...
    // Go to next step after consultation (manual mode)
    const nextIdx = STEPS_MANUAL.findIndex(s => s.id === 'consultation') + 1;
    setCurrentStep(nextIdx >= 1 ? nextIdx : 4);
  };
  ...
  const handleAutoGenerationComplete = (project: any) => {
    ...
    setIsAutoGenerating(false);
    // Navigate to preview step by ID
    const previewIdx = STEPS_FULL_SERVICE.findIndex(s => s.id === 'preview');
    setCurrentStep(previewIdx !== -1 ? previewIdx : 6);
  ...
  const handleSwitchToManual = (partialProject: any) => {
    ...
    if (partialProject?.script) {
      const scriptIdx = STEPS_MANUAL.findIndex(s => s.id === 'script');
      setCurrentStep(scriptIdx !== -1 ? scriptIdx : 6);
    } else {
      const briefIdx = STEPS_MANUAL.findIndex(s => s.id === 'briefing');
      setCurrentStep(briefIdx !== -1 ? briefIdx : 4);
    }
  ...
  const handleRetry = () => {
    ...
    setTimeout(() => {
      if (consultationResult && generationMode === 'full-service') {
        ...
        setIsAutoGenerating(true);
        const genIdx = STEPS_FULL_SERVICE.findIndex(s => s.id === 'generating');
        setCurrentStep(genIdx !== -1 ? genIdx : 5);
      }
    }, 150);
  };
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {/* Preview fallback when project is missing */}
          {currentStepId === 'preview' && !autoGeneratedProject && (
            <div className="max-w-lg mx-auto text-center py-12 space-y-6">
