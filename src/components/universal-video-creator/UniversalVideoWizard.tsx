...
export function UniversalVideoWizard() {
  ...
  // ✅ Phase 12: Only recover from CURRENT generation attempt, not any old completed video
  const dbFallbackAttempted = useRef(false);
  useEffect(() => {
    const isStuckOnGenerating = currentStep === 4 && !isAutoGenerating && !autoGeneratedProject;
    const isPreviewWithoutProject = currentStep === 5 && !autoGeneratedProject;
    ...
    if ((isStuckOnGenerating || isPreviewWithoutProject) && user && !dbFallbackAttempted.current) {
      ...
      import('@/integrations/supabase/client').then(({ supabase }) => {
        // Phase 12: Only recover records from the current generation attempt
        let query = supabase
        ...
        // If we have a generationStartedAt, only look at records created after it
  ...
  // Select steps based on mode (pre-mode selection only shows category + mood)
  ...
  const handleCategoryConfirm = () => {
    if (selectedCategory) {
      setCurrentStep(1); // Move to mood selection
    }
  };

  const handleMoodConfirm = (config: MoodConfig) => {
    setMoodConfig(config);
    setCurrentStep(2); // Move to mode selection
  };
  ...
  const handleConsultationComplete = (result: UniversalConsultationResult) => {
    ...
    if (generationMode === 'full-service') {
      // ✅ Phase 12: Clear old project state before new generation
      setAutoGeneratedProject(null);
      activeProgressIdRef.current = null;
      generationStartedAtRef.current = new Date().toISOString();
      dbFallbackAttempted.current = false;
      console.log('[UniversalVideoWizard] Phase12: Cleared old project, starting fresh generation');
  ...
  const handleAutoGenerationComplete = (project: any) => {
    ...
    setIsAutoGenerating(false);
    setCurrentStep(5); // Move to preview step (index 5 in STEPS_FULL_SERVICE)
  ...
  const handleRetry = () => {
    ...
    // ✅ Phase 12: Clear old project on retry
    setAutoGeneratedProject(null);
    generationStartedAtRef.current = new Date().toISOString();
    dbFallbackAttempted.current = false;
