...
export function UniversalVideoWizard() {
  ...
  useEffect(() => {
    ...
    if ((isStuckOnGenerating || isPreviewWithoutProject) && user && !dbFallbackAttempted.current) {
      ...
      console.log('[UniversalVideoWizard] r54b: DB recovery triggered, step=', currentStep);
          ...
          .then(({ data }) => {
            if (data?.result_data) {
              ...
              if (recovered.outputUrl) {
                console.log('[UniversalVideoWizard] r54b: Recovered project from DB:', recovered.outputUrl);
                setAutoGeneratedProject(recovered);
                // If stuck on generating step, advance to preview
                if (currentStep === 4) {
                  setCurrentStep(5);
                }
              }
