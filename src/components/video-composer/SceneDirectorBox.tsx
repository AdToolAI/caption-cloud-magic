...
const LABELS = {
  ...
  de: { title: tx({ de: 'Szene aus Beschreibung', en: 'Scene from description', es: 'Escena de la descripción' }), desc: tx({ de: tx({ de: 'Beschreibe die Szene in deinen Worten. Der Director sucht passende Library-Assets, passt die Aktion an die Dauer an und schreibt den AI-Prompt.', en: 'Describe the scene in your own words. The director will find suitable library assets, adjust the action to the duration, and write the AI prompt.', es: 'Describe la escena con tus propias palabras. El director buscará recursos de biblioteca adecuados, ajustará la acción a la duración y escribirá el prompt de IA.' }), en: 'Describe the scene in your own words. The director will find suitable library assets, adjust the action to the duration, and write the AI prompt.', es: 'Describe la escena con tus propias palabras. El director buscará recursos de biblioteca adecuados, ajustará la acción a la duración y escribirá el prompt de IA.' }), placeholder: tx({ de: tx({ de: '2. Weltkrieg. Ein Soldat fährt einen Leopard-Panzer über eine Steinbrücke im Morgenrot.', en: 'World War 2. A soldier drives a Leopard tank over a stone bridge at dawn.', es: 'Segunda Guerra Mundial. Un soldado conduce un tanque Leopard sobre un puente de piedra al amanecer.' }), en: 'World War 2. A soldier drives a Leopard tank over a stone bridge at dawn.', es: 'Segunda Guerra Mundial. Un soldado conduce un tanque Leopard sobre un puente de piedra al amanecer.' }), generate: tx({ de: 'Szene generieren', en: 'Generate scene', es: 'generar escena' }), regenerate: 'Neu würfeln', loading: 'Suche in deiner Library…', applied: tx({ de: 'Szene übernommen', en: 'Scene taken over', es: 'Escena tomada' }), dropped: tx({ de: 'Gekürzt auf die Szenendauer', en: 'Shortened to the scene duration', es: 'Acortado a la duración de la escena.' }), followup: 'Folgeszene anlegen', missing: 'Nicht in deiner Library', generateAsset: 'Erzeugen', castMissing: tx({ de: 'Cast nicht in Action', en: 'Don\'t cast in action', es: 'No lances en acción' }), castMissingHint: tx({ de: tx({ de: 'Diese ausgewählten Charaktere bekommen keine sichtbare Aktion — der Renderer zeigt sonst nur ein Gesicht. Re-Roll erzwingt sie in der Szene.', en: 'These selected characters won\'t have a visible action — the renderer will only show a face. Re-Roll forces them into the scene.', es: 'Estos personajes seleccionados no tendrán una acción visible; el renderizador solo mostrará una cara. Re-Roll los fuerza a la escena.' }), en: 'These selected characters won\'t have a visible action — the renderer will only show a face. Re-Roll forces them into the scene.', es: 'Estos personajes seleccionados no tendrán una acción visible; el renderizador solo mostrará una cara. Re-Roll los fuerza a la escena.' }), forceCast: 'Cast in Action erzwingen', ghostDropped: 'Geister-Cast verworfen' },
