...
export default function VideoComposerDashboard() {
      ...
      try {
        ...
        const dbScenes: ComposerScene[] = data.map((row: any) => {
          ...
          return {
            ...
            dialogTurns: ((row as any).dialog_turns as any) ?? (local as any)?.dialogTurns ?? [],
            dialogVoices: ((row as any).dialog_voices as any) ?? local?.dialogVoices ?? {},
            characterVoiceId: ((row as any).character_voice_id as any) ?? (local as any)?.characterVoiceId ?? undefined,
  ...
  const applyTemplate = useCallback((tpl: MotionStudioTemplate) => {
    ...
    toast({
      title: tx({ de: 'Template übernommen', en: 'Template adopted', es: 'Plantilla adoptada' }),
  ...
  const storyboardTransition = useStoryboardTransition({
  ...
  const handleTabChange = useCallback(async (next: TabId, opts?: { skipAnalysis?: boolean }) => {
    ...
    if (next === 'storyboard' && activeTab === 'briefing') {
      ...
      if (handled) {
        ...
        // step will navigate via `onApplied` once the user confirms.
        return;
      }
    }
  ...
  }, [project.id, activeTab, storyboardTransition]);
  ...
  const insertScenesAfter = useCallback(async (
    ...
    const childrenPayload = partials.map((p) => ({
      ...
      dialog_turns: ((p as any).dialogTurns ?? []) as any,
          ...
          <TabsContent value="storyboard">
            <ComposerTabErrorBoundary label="Storyboard">
              <StoryboardTab
                ...
                briefingTargetDurationSec={project.briefing?.duration}
