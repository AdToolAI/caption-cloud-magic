...
const T = {
  de: {
    ...
    voiceMissing: (name: string) => tx({ de: tx({ de: tx({ de: `Wähle eine Stimme für „${name}".`, en: `Choose a voice for "${name}".`, es: `Elige una voz para "${name}".` }), en: `Choose a voice for "${name}".`, es: `Elige una voz para "${name}".` }), en: `Choose a voice for "${name}".`, es: `Elige una voz para "${name}".` }),
    parseEmpty: 'Kein gültiges Skript. Format: "Sarah: Hallo!"',
    success: (n: number) => tx({ de: tx({ de: tx({ de: `${n} Lip-Sync-Clip${n === 1 ? '' : 'stx({ de: '} werden generiert (1–3 Min).`, en: `${n} Lip Sync Clip${n === 1 ? '' : 'stx({ de: '} are generated (1–3 min).`, es: `${n} Clip de sincronización de labios${n === 1 ? '' : 'stx({ de: '} se generan (1–3 min).` }), en: `${n} Lip Sync Clip${n === 1 ? ', en: '} are generated (1–3 min).`, en: `${n} Lip Sync Clip${n === 1 ?', es: '} se generan (1–3 min).`, en: `${n} Clip de sincronización de labios${n === 1 ?' })' : 's'} are generated (1-3 min).`, es: `${n} Clip de sincronización de labios${n === 1 ? '' : 's'} se generan (1-3 min).` }), en: `${n} Lip Sync Clip${n === 1 ? '' : 's'} are generated (1-3 min).`, es: `${n} Clip de sincronización de labios${n === 1 ? '' : 's'} se generan (1-3 min).` }),
    ...
    continuityTooltipInherited: (n: number) => tx({ de: tx({ de: tx({ de: `Sarah/Matthew werden visuell an Szene ${n} angeglichen.`, en: `Sarah/Matthew are visually matched to scene ${n}.`, es: `Sarah/Matthew se ajustan visualmente a la escena ${n}.` }), en: `Sarah/Matthew are visually matched to scene ${n}.`, es: `Sarah/Matthew se ajustan visualmente a la escena ${n}.` }), en: `Sarah/Matthew are visually matched to scene ${n}.`, es: `Sarah/Matthew se ajustan visualmente a la escena ${n}.` }),
...
const SceneDialogStudio = forwardRef<HTMLDivElement, SceneDialogStudioProps>(function SceneDialogStudio({
  ...
  const handleGenerate = async () => {
      ...
      for (const sp of speakers) {
        ...
        if (seen.has(vid)) {
          ...
          toast({
            ...
            description: tx({ de: tx({ de: tx({ de: `${other} und ${sp.name} nutzen dieselbe Stimme. Bitte unterschiedliche Stimmen wählen.`, en: `${other} and ${sp.name} use the same voice. Please choose different voices.`, es: `${other} y ${sp.name} usan la misma voz. Por favor, elige voces diferentes.` }), en: `${other} and ${sp.name} use the same voice. Please choose different voices.`, es: `${other} y ${sp.name} usan la misma voz. Por favor, elige voces diferentes.` }), en: `${other} and ${sp.name} use the same voice. Please choose different voices.`, es: `${other} y ${sp.name} usan la misma voz. Por favor, elige voces diferentes.` }),
            variant: 'destructive',
          });
          return;
        }
    ...
    if (blocks.length === 1 && !allHavePortraits) {
      ...
      toast({
        title: tx({ de: tx({ de: tx({ de: `Kein Portrait für ${name}`, en: `No portrait for ${name}`, es: `Sin retrato para ${name}` }), en: `No portrait for ${name}`, es: `Sin retrato para ${name}` }), en: `No portrait for ${name}`, es: `Sin retrato para ${name}` }),
            ...
            if (blocks.length >= 2) {
              ...
              return willSrs ? (
                <>
                  ...
                  {tx({ de: tx({ de: tx({ de: `🎭 ${blocks.length} Dialog-Shots in echte Szene rendern`, en: `🎭 Render ${blocks.length} dialogue shots into real scene`, es: `🎭 Renderiza tomas de diálogo de ${blocks.length} en una escena real` }), en: `🎭 Render ${blocks.length} dialogue shots into real scene`, es: `🎭 Renderiza tomas de diálogo de ${blocks.length} en una escena real` }), en: `🎭 Render ${blocks.length} Dialog Shots into real scene`, es: `🎭 Renderizar ${blocks.length} Dialog-Shots en escena real` })}
                </>
              ) : (
                <>
