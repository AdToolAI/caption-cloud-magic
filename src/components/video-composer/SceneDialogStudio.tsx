...
const SceneDialogStudio = forwardRef<HTMLDivElement, SceneDialogStudioProps>(function SceneDialogStudio({
  ...
  const handleGenerate = async () => {
    ...
    if (blocks.length >= 2 && !allHavePortraits) {
      ...
      if (missingPortrait) {
        toast({
          title: tx({ de: `Kein Portrait für ${missingPortrait.name}`, en: `No portrait for ${missingPortrait.name}`, es: `No hay retrato para ${missingPortrait.name}` }),
            ...
            if (blocks.length >= 2) {
              ...
              return willSrs ? (
                <>
                  ...
                  {tx({ de: `🎭 ${blocks.length} Dialog-Shots in echte Szene rendern`, en: `🎭 Render ${blocks.length} dialogue shots into real scene`, es: `🎭 Renderiza tomas de diálogo de ${blocks.length} en una escena real` })}
                </>
              ) : (
                <>
