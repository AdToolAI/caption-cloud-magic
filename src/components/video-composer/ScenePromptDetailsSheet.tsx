...
const t = {
  de: {
    title: tx({ de: 'Prompt-Details', en: 'Prompt details', es: 'Detalles del prompt' }),
    description: tx({ de: 'Was die KI tatsächlich bekommt — Live-Prompt, Layer-Aufschlüsselung, Multi-Engine-Vergleich.', en: 'What the AI actually receives — Live prompt, layer breakdown, multi-engine comparison.', es: 'Lo que realmente recibe la IA: prompt en vivo, desglose de capas, comparación de varios motores.' }),
    livePrompt: tx({ de: 'Live Prompt (so wird er an die KI geschickt)', en: 'Live Prompt (how it is sent to the AI)', es: 'Prompt en vivo (cómo se envía a la IA)' }),
    finalPrompt: tx({ de: 'Komponierter Final-Prompt', en: 'Composed final prompt', es: 'Prompt final compuesto' }),
    layers: tx({ de: 'Layer-Details', en: 'Layer details', es: 'Detalles de la capa' }),
    layersHide: tx({ de: 'Details ausblenden', en: 'Hide details', es: 'Ocultar detalles' }),
    multiEngine: tx({ de: 'Multi-Engine Vorschau', en: 'Multi-engine preview', es: 'Vista previa multimotor' }),
    multiEngineShow: tx({ de: 'Multi-Engine Vorschau anzeigen', en: 'Show multi-engine preview', es: 'Mostrar vista previa multimotor' }),
    multiEngineHide: tx({ de: 'Multi-Engine ausblenden', en: 'Hide multi-engine', es: 'Ocultar multimotor' }),
    compare: tx({ de: 'Auf Engines vergleichen', en: 'Compare on engines', es: 'Comparar en motores' }),
    deduped: tx({ de: 'entfernt (Dedup)', en: 'removed (dedup)', es: 'eliminado (dedup)' }),
    nothing: tx({ de: 'Noch kein Prompt — schreibe etwas in die Textarea.', en: 'No prompt yet — type something in the textarea.', es: 'Aún no hay ningún prompt; escribe algo en el área de texto.' }),
  },
  en: {
