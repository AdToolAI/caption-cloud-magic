...
export function SpeechDurationHint({ text, targetDurationSec, language = 'de' }: SpeechDurationHintProps) {
  const { words, seconds, tone, message, suppress } = useMemo(() => {
    ...
    if (words < 6) {
      ...
      tone = 'ok';
      message = tx({ de: `Passt zu ${targetDurationSec}s Gesamtdauer.`, en: `Fits the ${targetDurationSec}s total duration.`, es: `Se ajusta a la duración total de ${targetDurationSec}s.` });
    } else if (ratio <= 1.4) {
      tone = 'warn';
