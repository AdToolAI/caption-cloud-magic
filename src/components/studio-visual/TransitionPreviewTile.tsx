...
import sceneA from '@/assets/studio-presets/framing/establishing.jpg';
import sceneB from '@/assets/studio-presets/framing/wide.jpg';
import './motionTiles.css';

export type TransitionId =
  | 'none' | 'fade' | 'crossfade' | 'dissolve'
  | 'wipe' | 'slide' | 'blur' | 'zoom' | 'push' | 'morph';
...
/**
 * Locked-base-scene tile that loops the actual transition between TWO fixed
 * scenes (Tokyo Establishing → Tokyo Wide). Loops only on hover or when
 * active so idle CPU stays low.
 */
export const TransitionPreviewTile: React.FC<TransitionPreviewTileProps> = ({
  transitionId,
  label,
  isActive = false,
  onClick,
  ...
  const cls = `tx-${transitionId}`;
  return (
    <button
      type="button"
      onClick={onClick}
      onMouseEnter={(e) => e.currentTarget.querySelector('.tx-stage')?.setAttribute('data-play', 'true')}
      onMouseLeave={(e) => {
        const stage = e.currentTarget.querySelector('.tx-stage');
    ...
    >
      <div
        className={cn('tx-stage relative w-full overflow-hidden bg-black', dim, cls)}
        data-play={isActive ? 'true' : 'false'}
      >
        <img src={sceneA} alt="" aria-hidden className="tx-a" draggable={false} />
        <img src={sceneB} alt={label} className="tx-b" draggable={false} />
...
export default TransitionPreviewTile;
