...
import { HAILUO_VIDEO_MODELS, HailuoVideoModel, HailuoResolution } from '@/config/hailuoVideoCredits';
...
export default function HailuoVideoStudio() {
  const { user } = useAuth();
  ...
  const modelConfig = HAILUO_VIDEO_MODELS[model];
  ...
  const uploadFile = async (file: File): Promise<string | null> => {
    ...
    const path = `${user.id}/hailuo-${Date.now()}.${ext}`;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.success(`Hailuo 2.3 Video wird generiert! Kosten: ${currencySymbol}${cost.toFixed(2)}`);
      refetchWallet();
  ...
  return (
    <>
      <Helmet>
        <title>Hailuo 2.3 Video Studio | AI Video Generator</title>
        <meta name="description" content="Generate realistic AI videos with MiniMax Hailuo 2.3" />
      </Helmet>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold">Hailuo 2.3 Video Studio</h1>
              <Badge variant="secondary" className="bg-primary/10 text-primary border-primary/20">Neu</Badge>
            </div>
            <p className="text-muted-foreground text-sm mt-1">
              Realistic Motion & Characters • 6 oder 10 Sekunden • bis 1080p
            </p>
          </div>
          <div className="flex items-center gap-3">
              ...
              <div className="space-y-5">
                ...
                <Card className="p-4">
                  ...
                  <Textarea
                    ...
                    placeholder="A woman with flowing hair walks through a sunlit garden, realistic close-up, shallow depth of field, cinematic color grading"
                  ...
                  <div className="mt-2 p-2 rounded-md bg-muted/50 border border-border/50">
                    <p className="text-xs text-muted-foreground">
                      💡 <strong>Tipp:</strong> Hailuo 2.3 ist besonders stark bei realistischen Gesichtern und Charakterbewegungen. Beschreibe Personen und deren Aktionen für beste Ergebnisse.
                    </p>
                  </div>
                </Card>
                ...
                <Card className="p-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {(Object.entries(HAILUO_VIDEO_MODELS) as [HailuoVideoModel, typeof HAILUO_VIDEO_MODELS[HailuoVideoModel]][]).map(([key, m]) => (
                      <button
                        key={key}
                ...
                <Card className="p-4">
                  ...
                  <ToggleGroup
                    ...
                    className="w-full"
                  >
                    <ToggleGroupItem value="6" className="flex-1 data-[state=on]:bg-primary data-[state=on]:text-primary-foreground">
                      6 Sekunden
                    </ToggleGroupItem>
                ...
                <Card className="p-4">
                  <Label className="text-sm font-medium mb-3 block flex items-center gap-1.5">
                    <Monitor className="h-4 w-4" />Auflösung
                  </Label>
                  <div className="flex gap-2">
                    <Button
                      variant={resolution === '768p' ? 'default' : 'outline'}
                      size="sm"
                    ...
                    >
                      1080p HD {duration === 10 && '(nur 6s)'}
                    </Button>
                  </div>
                  {duration === 10 && (
                    <p className="text-xs text-muted-foreground mt-2">⚠️ 1080p ist nur bei 6 Sekunden Dauer verfügbar.</p>
                  )}
                </Card>
                ...
                <Card className="p-4">
                  ...
                  <div>
                    <p className="text-xs text-muted-foreground mb-2">Startbild hochladen (First Frame)</p>
                    {startImageUrl ? (
              ...
              <div className="space-y-4">
                ...
                <Card className="p-4">
                  ...
                  <div className="text-xs space-y-1 text-muted-foreground">
                    <div className="flex justify-between"><span>Standard 6s</span><span>{currencySymbol}0.90</span></div>
                    <div className="flex justify-between"><span>Standard 10s</span><span>{currencySymbol}1.50</span></div>
                    <hr className="my-1" />
                    <div className="flex justify-between"><span>Pro 6s</span><span>{currencySymbol}1.20</span></div>
