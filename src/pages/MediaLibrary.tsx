...
export default function MediaLibrary() {
        ...
        return {
          ...
          title: isSoraAI 
            ...
            : isDirectorCutEnhancement
              ? tx({ de: tx({ de: `KI-Szene: ${metadata?.prompt?.slice(0, 40) || 'Enhancement'}...`, en: `AI scene: ${metadata?.prompt?.slice(0, 40) || 'Enhancement'}...`, es: `Escena de IA: ${metadata?.prompt?.slice(0, 40) || 'Realce'}...` }), en: `AI scene: ${metadata?.prompt?.slice(0, 40) || 'Enhancement'}...`, es: `Escena de IA: ${metadata?.prompt?.slice(0, 40) || 'Realce'}...` })
              : isDirectorsCut
                ? `Director's Cut - ${new Date(video.created_at).toLocaleDateString('de-DE')}`
                : isMotionStudioClip
                  ? tx({ de: tx({ de: `Motion Studio · Szene ${(metadata?.scene_order ?? 0) + 1}${isSuperseded ? ' (Vorgängerversion)' : ''}`, en: `Motion Studio · Scene ${(metadata?.scene_order ?? 0) + 1}${isSuperseded ? ' (previous version)' : ''}`, es: `Motion Studio · Escena ${(metadata?.scene_order ?? 0) + 1}${isSuperseded ? ' (versión anterior)' : ''}` }), en: `Motion Studio · Scene ${(metadata?.scene_order ?? 0) + 1}${isSuperseded ? ' (previous version)' : ''}`, es: `Motion Studio · Escena ${(metadata?.scene_order ?? 0) + 1}${isSuperseded ? ' (versión anterior)' : ''}` })
                  : isUniversalCreator
  ...
  const sendToCalendar = (mediaItem: NormalizedMediaItem) => {
    sessionStorage.setItem('calendar_prefill', JSON.stringify({
      title: mediaItem.title || tx({ de: tx({ de: `Post vom ${new Date().toLocaleDateString('de-DE')}`, en: `Post from ${new Date().toLocaleDateString('de-DE')}`, es: `Publicar desde ${new Date().toLocaleDateString('de-DE')}` }), en: `Post from ${new Date().toLocaleDateString('de-DE')}`, es: `Publicar desde ${new Date().toLocaleDateString('de-DE')}` }),
  ...
  const bulkSendToAIPostGenerator = () => {
    ...
    toast({
      title: "📸 Media gesendet",
      description: tx({ de: tx({ de: `Erstes Medium wird im KI-Post-Generator geladen...`, en: `First medium is loaded in the AI ​​post generator...`, es: `El primer medio se carga en el generador de publicaciones de IA...` }), en: `First medium is loaded in the AI ​​post generator...`, es: `El primer medio se carga en el generador de publicaciones de IA...` }),
    });
  ...
  const bulkSendToCalendar = () => {
    ...
    sessionStorage.setItem('calendar_prefill', JSON.stringify({
      title: firstItem.title || tx({ de: tx({ de: `Post vom ${new Date().toLocaleDateString('de-DE')}`, en: `Post from ${new Date().toLocaleDateString('de-DE')}`, es: `Publicar desde ${new Date().toLocaleDateString('de-DE')}` }), en: `Post from ${new Date().toLocaleDateString('de-DE')}`, es: `Publicar desde ${new Date().toLocaleDateString('de-DE')}` }),
    ...
    toast({
      title: "📅 Media gesendet",
      description: tx({ de: tx({ de: `Erstes Medium wird im Kalender geladen...`, en: `First medium is loaded in the calendar...`, es: `El primer medio está cargado en el calendario...` }), en: `First medium is loaded in the calendar...`, es: `El primer medio está cargado en el calendario...` }),
    });
  ...
  const bulkSendToBackgroundReplacer = () => {
    ...
    toast({
      title: "🎨 Media gesendet",
      description: tx({ de: tx({ de: `Erstes Bild wird in Smart Background geladen...`, en: `First image is loading into Smart Background...`, es: `La primera imagen se está cargando en Smart Background...` }), en: `First image is loading into Smart Background...`, es: `La primera imagen se está cargando en Smart Background...` }),
    });
