...
export function useMusicAutoMatch() {
  ...
  const analyzeVideo = useCallback(async (file: File): Promise<AutoMatchResult | null> => {
    ...
    try {
      ...
      if (upErr) throw new Error(tx({ de: tx({ de: `Upload fehlgeschlagen: ${upErr.message}`, en: `Upload failed: ${upErr.message}`, es: `Error al cargar: ${upErr.message}` }), en: `Upload failed: ${upErr.message}`, es: `Error al subir: ${upErr.message}` }));
