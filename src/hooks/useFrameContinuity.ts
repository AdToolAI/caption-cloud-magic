  ...
  const extractLastFrame = useCallback(
    async (params: {
      ...
      try {
        ...
        if (upErr) throw new Error(tx({ de: tx({ de: tx({ de: `Upload fehlgeschlagen: ${upErr.message}`, en: `Upload failed: ${upErr.message}`, es: `Error al cargar: ${upErr.message}` }), en: `Upload failed: ${upErr.message}`, es: `Error al cargar: ${upErr.message}` }), en: `Upload failed: ${upErr.message}`, es: `Error al cargar: ${upErr.message}` }));
