...
export function useStoryboardTransition({
  ...
  const attempt = useCallback(async (opts?: { force?: boolean }): Promise<{ handled: boolean }> => {
    ...
    try {
      ...
      while (true) {
        try {
          // v414 — single request per attempt (Parallel-Fire entfernt: er hat
          // die Deep-Analyse faktisch immer doppelt ausgeführt). Netzabbrüche
          // und 5xx fängt die Retry-Schleife unten ab.
          res = await doFetch();
        } catch (fetchErr: any) {
      ...
      if (dropped > 0) {
        toast({
          title: tx({ de: 'Plan teilweise übernommen', en: 'Plan partially applied', es: 'Plan aplicado parcialmente' }),
