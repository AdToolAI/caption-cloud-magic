...
function detectCanonicalBriefingTiming(briefing: ComposerBriefing, briefingText: string): BriefingTimingWithWindows | null {
  ...
  const explicitTotalPatterns = [
    /(?:gesamt\s*dauer|gesamtdauer|gesamt\s*länge|gesamtlaenge|gesamtlänge|total\s*duration|filmdauer|film\s*dauer|video\s*dauer|spot\s*dauer|laufzeit|länge|laenge|dauer|duration)\s*[:=\-–—]?\s*(?:ca\.?\s*)?(\d+(?:[,.]\d+)?)\s*(?:sekunden|sek\.?|seconds|secs?|s)\b/i,
