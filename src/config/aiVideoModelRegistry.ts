...
export interface ToolkitModel {
  ...
  /**
   * DERIVED — never hand-maintained here.
   * Allowed durations in seconds, taken from the canonical capability
   * registry (`videoModelSpecs`) at module load.
   */
  durations: number[];
  /** DERIVED — highest startable native resolution label (badge default). */
  resolution: string;
  /** DERIVED — all startable native resolution labels, highest first. */
  resolutions?: string[];
  /** DERIVED — aspect ratios the canonical registry documents. */
  aspectRatios: string[];
...
/**
 * Metadata-only entry: labels, icons, taglines, grouping, pricing, routing.
 * The technical capabilities (durations, native resolutions, aspect ratios,
 * modes, audio, smart duration) are NOT maintained here — they are derived
 * from the canonical capability registry in `deriveTechnicalCapabilities()`.
 */
type ToolkitModelMeta = Omit<ToolkitModel, 'durations' | 'resolution' | 'resolutions' | 'aspectRatios'>;

const AI_VIDEO_TOOLKIT_MODELS_RAW: ToolkitModelMeta[] = [
 ...
 */
/**
 * Technical truth comes from ONE place: the generated mirror of the canonical
 * capability registry. Modes, native resolutions (locked tiers excluded),
 * durations, aspect ratios, audio and smart duration are derived here, so the
 * UI can never offer an option the server gate would reject.
 */
function deriveTechnicalCapabilities(m: ToolkitModelMeta): ToolkitModel {
  const union = getModelCapabilityUnion(m.id);
  if (!union.supported) {
    throw new Error(
      `[aiVideoModelRegistry] "${m.id}" has no entry in the canonical capability registry (videoModelSpecs).`,
    );
  }
