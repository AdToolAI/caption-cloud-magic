...
describe('AI Video model registry ↔ pricing catalog', () => {
  ...
  it('Seedance 2.5 sells at 19.90 EUR per 30 s clip', () => {
    const s25 = AI_VIDEO_TOOLKIT_MODELS.find((m) => m.id === 'seedance-2-5')!;
    expect(s25.costPerSecond.EUR * 30).toBeCloseTo(19.9, 1);
    expect(VIDEO_PRICING_CATALOG['seedance-2-5'].sellEUR * 30).toBeCloseTo(19.9, 1);
  });
});
