// AUTO-GENERATED: fills keys that exist in one dictionary but were missing
// from the others. Deep-merged in useTranslation, so every locale resolves
// its own copy instead of falling back to another language.
export const translationsFill: Record<'en' | 'de' | 'es', any> = {
 "en": {
  "calendar": {
   "messages": {
    "copySuccess": "Link copied",
    "exportSuccess": "Export successful",
    "loadFailed": "Could not load events",
    "moveFailed": "Move failed",
    "statusFailed": "Status update failed",
    "statusUpdated": "Status updated",
    "noEventsToExport": "No events to export",
    "pdfPrintDialog": "Opening print dialog for PDF export...",
    "saving": "Saving...",
    "createEventComingSoon": "Event creation coming soon",
    "addNoteComingSoon": "Add note coming soon",
    "filterComingSoon": "Filter function coming soon",
    "shareComingSoon": "Share function coming soon",
    "workspaceCreated": "Workspace created successfully",
    "defaultWorkspace": "My Workspace"
   },
   "autoSchedule": {
    "description": "AI suggests optimal time slots",
    "analyze": "Analyze",
    "applyAll": "Accept all",
    "bestTime": "Best time",
    "reason": "Reason"
   },
   "api": {
    "errors": {
     "MISSING_REQUIRED_FIELDS": "Required fields missing",
     "TEMPLATE_NOT_FOUND": "Template not found",
     "NO_POSTS_IN_RANGE": "No posts in period",
     "POST_NOT_FOUND": "Post not found",
     "SCHEDULE_CONFLICT": "Schedule conflict",
     "GENERATION_FAILED": "Generation failed",
     "EXPORT_FAILED": "Export failed"
    },
    "success": {
     "POSTS_SCHEDULED": "{count} posts scheduled",
     "SCHEDULE_APPLIED": "Schedule applied",
     "CAMPAIGN_CREATED": "Campaign with {count} events created",
     "EVENT_RESCHEDULED": "Event rescheduled",
     "EXPORT_READY": "Export ready"
    },
    "timeQuality": {
     "BEST_TIME": "Best time"
    }
   },
   "actions": {
    "addNote": "Add note"
   }
  },
  "pricing": {
   "upgrade": {
    "toEnterprise": "Upgrade to Enterprise"
   }
  },
  "pricingPage": {
   "plans": {
    "enterprise": {
     "badge": "Top tier"
    }
   }
  },
  "newsHub": {
   "streak": {
    "cardTitle": "Active streak",
    "pageTitle": "Your streak",
    "pageDescription": "Stay active every day to grow your streak and unlock rewards.",
    "current": "Current",
    "longest": "Longest",
    "totalDays": "Total days",
    "last30Days": "Last 30 days",
    "last90Days": "Last 90 days",
    "daysStreak": "day streak",
    "days": "days",
    "viewDetails": "Details",
    "freezeTokens": "{{count}} freeze",
    "freezeTokensLabel": "Freeze tokens",
    "nextMilestone": "{{remaining}}d to {{next}}",
    "nextMilestoneLong": "{{remaining}} more days to reach {{next}} days",
    "tooltipWithNext": "{{current}} days in a row — {{remaining}} more for the {{next}}-day reward",
    "tooltipMax": "{{current}} days in a row — legendary streak!",
    "milestoneReached": "🔥 {{days}}-day streak reached!",
    "milestoneCredits": "+{{credits}} bonus credits added to your account",
    "milestoneDollars": "+${{amount}} in AI Video credits added to your account 🎬",
    "milestoneDollarsShort": "+${{amount}} AI Video credits",
    "milestonesTitle": "Milestones",
    "noMilestonesYet": "No milestones yet — keep going!"
   },
   "upgrade": {
    "smart": {
     "credits": {
      "title": "Only {count} credits left",
      "body": "You are running low on credits. Upgrade to {plan} for a fresh monthly bundle so you never stop mid-project."
     },
     "feature": {
      "title": "{feature} is a {plan} feature",
      "body": "Unlock {feature} and many more pro tools by upgrading to {plan}."
     },
     "streak": {
      "title": "🔥 {days}-day streak — you are unstoppable!",
      "body": "Power users like you save more with {plan}. More credits, premium features and exclusive perks."
     },
     "usage": {
      "title": "You are using AdTool heavily",
      "body": "Based on your activity, {plan} would give you more credits and unlock the features you need next."
     },
     "manual": {
      "title": "Unlock more with {plan}",
      "body": "Upgrade to {plan} to access the full set of pro features."
     },
     "annualSaving": "Save 17% per year",
     "perfectForYou": "Perfect for power users",
     "maybeLater": "Maybe later",
     "upgradeCta": "Upgrade now",
     "benefit": {
      "credits": "{count} credits per month",
      "unlimited": "Unlimited credits",
      "posting": "Auto-posting to every platform",
      "team": "Team collaboration and workspaces",
      "analytics": "Advanced analytics dashboard",
      "brands": "Unlimited brand kits",
      "api": "Full API access",
      "priority": "Priority support"
     },
     "trial": {
      "halfwayTitle": "You are halfway through your trial — {days} days left",
      "endingTitle": "Only {days} days of Enterprise left",
      "lastDayTitle": "Last day of your Enterprise trial!",
      "graceTitle": "Trial expired — account pauses in {days} days",
      "body": "Secure {plan} now and keep every premium feature without interruption.",
      "lastDayBody": "Do not lose access tomorrow — get {plan} with 20% off your first month.",
      "graceBody": "You still have access during the grace period. Upgrade to {plan} now to avoid the pause.",
      "couponLabel": "Your trial discount",
      "couponDiscount": "20% off the first month",
      "cta": "Upgrade with TRIAL20"
     },
     "discovery": {
      "title": "You love {feature} — get unlimited access",
      "body": "You have used {feature} several times. {plan} unlocks unlimited renders and priority processing."
     }
    }
   },
   "verifyEmail": {
    "loading": "Verifying your email…",
    "pleaseWait": "Please wait",
    "success": "Verified successfully! 🎉",
    "successDesc": "Your email address is confirmed. You now have full access to every feature.",
    "error": "Verification failed",
    "noToken": "No verification token found",
    "expired": "This verification link is invalid or has expired. Please request a new email.",
    "generic": "Verification failed. Please try again.",
    "goDashboard": "Go to dashboard",
    "resend": "Send a new email",
    "backToLogin": "Back to sign in",
    "resentTitle": "New verification email sent!",
    "resentDesc": "Check your inbox",
    "resendError": "Sending failed",
    "pleaseLogin": "Please sign in again"
   },
   "emailGate": {
    "badge": "Email verification required",
    "title": "Verify your email",
    "subtitle": "To use AdTool AI in full, please confirm your email address.",
    "emailLabel": "Email address",
    "instructions": "We sent you a verification email. Click the link to activate your account.",
    "checkSpam": "Also check your spam or junk folder",
    "resend": "Resend verification email",
    "resendIn": "Resend in {seconds}s",
    "refresh": "Refresh status",
    "sentTitle": "Email sent!",
    "sentDesc": "Check your inbox and spam folder",
    "sendError": "Sending failed",
    "tryAgain": "Please try again later",
    "refreshError": "Refresh failed",
    "verified": "Email verified!",
    "welcome": "Welcome to AdTool AI!",
    "notYet": "Not verified yet",
    "clickLink": "Click the link in the email",
    "problemsContact": "Having trouble? Contact our",
    "support": "Support"
   },
   "checkEmail": {
    "title": "Check your email",
    "subtitle": "We sent you a verification link to activate your account.",
    "sentTo": "Email sent to:",
    "step1": "Open the email from AdTool AI",
    "step2": "Click the verification link",
    "step3": "Remember to check your spam folder",
    "resend": "Resend email",
    "resendIn": "Resend in {seconds}s",
    "useDifferent": "Use a different email",
    "resentTitle": "Email resent!",
    "resentDesc": "Check your inbox and spam folder",
    "resendError": "Resending failed",
    "notLoggedIn": "Sign in again to resend",
    "problemsContact": "Need help? Contact our",
    "support": "Support"
   }
  }
 },
 "de": {
  "analytics": {
   "empty": {
    "title": "Noch keine Analysedaten",
    "body": "Synchronisiere die letzten 30 Tage, um deine Performance zu sehen.",
    "cta": "Jetzt synchronisieren"
   }
  },
  "mediaLibrary": {
   "title": "Medienbibliothek",
   "subtitle": "Verwalte alle deine Content-Assets an einem Ort",
   "upload": "Datei hochladen",
   "uploadSuccess": "Datei erfolgreich hochgeladen",
   "deleteSuccess": "Datei erfolgreich gelöscht",
   "searchPlaceholder": "Dateien suchen …",
   "fileType": "Dateityp",
   "allTypes": "Alle Typen",
   "images": "Bilder",
   "videos": "Videos",
   "documents": "Dokumente",
   "noMedia": "Noch keine Mediendateien",
   "uploadFirst": "Lade deine erste Datei hoch, um zu starten"
  },
  "search": "Suchen",
  "email": "E-Mail",
  "error": "Fehler",
  "success": "Erfolg",
  "cancel": "Abbrechen",
  "hubs": {
   "einstellungen": "Einstellungen"
  },
  "onb": {
   "accounts": "Konten verbinden",
   "brandkit": "Brand Kit einrichten",
   "goal": "Ziel festlegen",
   "plan": "1-Wochen-Plan erstellen",
   "automation": "Auto-Posting aktivieren",
   "planGenerated": "Dein 1-Wochen-Plan wurde erstellt!"
  },
  "heatmap": {
   "empty": {
    "cta": "Konten verbinden"
   }
  },
  "wl": {
   "poweredByDesc": "AdTool-AI-Badge im Footer anzeigen"
  },
  "calendar": {
   "autoSchedule": {
    "subtitle": "Lass die KI die besten Zeiten für deinen Content finden",
    "selectEvents": "Events auswählen",
    "noEvents": "Keine Events ausgewählt",
    "generate": "Zeitplan erstellen",
    "generating": "Beste Zeiten werden analysiert …",
    "apply": "Vorschläge übernehmen",
    "noSuggestions": "Keine Vorschläge verfügbar",
    "success": "Events erfolgreich geplant",
    "error": "Zeitplan konnte nicht erstellt werden",
    "reasons": {
     "best_time": "Optimale Posting-Zeit",
     "high_engagement": "Fenster mit hoher Interaktion",
     "low_competition": "Slot mit wenig Konkurrenz",
     "audience_active": "Zielgruppe am aktivsten"
    }
   },
   "templates": {
    "title": "Kampagnen-Vorlagen",
    "subtitle": "Kampagnen aus fertigen Vorlagen erstellen",
    "select": "Vorlage auswählen",
    "campaignName": "Kampagnenname",
    "startDate": "Startdatum",
    "generate": "Kampagne erstellen",
    "generating": "Kampagne wird erstellt …",
    "preview": "Vorschau",
    "duration": "Dauer",
    "postsCount": "Beiträge",
    "success": "Kampagne erfolgreich erstellt",
    "error": "Kampagne konnte nicht erstellt werden",
    "types": {
     "launch": "Produkt-Launch",
     "sale": "Verkaufskampagne",
     "season": "Saisonaler Content",
     "always_on": "Always-On-Content"
    }
   },
   "blackout": {
    "subtitle": "Termine sperren, an denen nicht gepostet werden darf",
    "startTime": "Startzeit",
    "endTime": "Endzeit",
    "save": "Speichern",
    "success": "Sperrtermin hinzugefügt",
    "error": "Sperrtermin konnte nicht hinzugefügt werden",
    "reasons": {
     "holiday": "Feiertag",
     "maintenance": "Systemwartung",
     "event": "Besonderes Event",
     "other": "Sonstiges"
    }
   },
   "api": {
    "success": {
     "scheduled": "Beiträge erfolgreich geplant",
     "campaignGenerated": "Kampagne erfolgreich erstellt",
     "blackoutAdded": "Sperrtermin hinzugefügt",
     "holidaysFetched": "Feiertags-Ideen erstellt"
    },
    "errors": {
     "BLACKOUT_CONFLICT": "Diese Zeit kollidiert mit einem Sperrtermin"
    },
    "timeQuality": {
     "high": "Sehr guter Zeitslot",
     "medium": "Guter Zeitslot",
     "low": "Akzeptabler Zeitslot"
    }
   },
   "copySuccess": "Link in die Zwischenablage kopiert",
   "exportSuccess": "Export erfolgreich",
   "loadFailed": "Events konnten nicht geladen werden",
   "moveFailed": "Event konnte nicht verschoben werden",
   "statusFailed": "Status konnte nicht aktualisiert werden",
   "statusUpdated": "Status aktualisiert",
   "noEventsToExport": "Keine Events zum Exportieren",
   "pdfPrintDialog": "Druckdialog für den PDF-Export wird geöffnet …",
   "saving": "Wird gespeichert …",
   "createEventComingSoon": "Event-Erstellung kommt bald",
   "addNoteComingSoon": "Notiz hinzufügen kommt bald",
   "filterComingSoon": "Filterfunktion kommt bald",
   "shareComingSoon": "Teilen-Funktion kommt bald",
   "workspaceCreated": "Workspace erfolgreich erstellt",
   "defaultWorkspace": "Mein Workspace",
   "apiDuplicate": {
    "errors": {
     "UNAUTHORIZED": "Nicht autorisiert",
     "MISSING_REQUIRED_FIELDS": "Pflichtfelder fehlen",
     "TEMPLATE_NOT_FOUND": "Vorlage nicht gefunden",
     "NO_DRAFTS_AVAILABLE": "Keine Entwürfe verfügbar",
     "NO_POSTS_IN_RANGE": "Keine Beiträge im Zeitraum",
     "POST_NOT_FOUND": "Beitrag nicht gefunden",
     "SCHEDULE_CONFLICT": "Terminkonflikt",
     "CAPACITY_EXCEEDED": "Kapazität überschritten",
     "GENERATION_FAILED": "Erstellung fehlgeschlagen",
     "EXPORT_FAILED": "Export fehlgeschlagen",
     "INTERNAL_ERROR": "Interner Fehler"
    },
    "success": {
     "POSTS_SCHEDULED": "{count} Beiträge geplant",
     "SCHEDULE_APPLIED": "Zeitplan übernommen",
     "CAMPAIGN_CREATED": "Kampagne mit {count} Events erstellt",
     "EVENT_RESCHEDULED": "Event neu geplant",
     "EXPORT_READY": "Export bereit"
    },
    "timeQuality": {
     "BEST_TIME": "Beste Zeit",
     "GOOD_TIME": "Gute Zeit"
    }
   },
   "actions": {
    "addNote": "Notiz hinzufügen"
   }
  },
  "pricing": {
   "quickPost": {
    "locked": {
     "title": "Auto-Planung gesperrt",
     "description": "Quick Post im Kalender ist ab dem Pro-Plan verfügbar (2.500 Credits/Monat)."
    },
    "includedIn": "Enthalten in"
   }
  },
  "pricingPage": {
   "plans": {
    "enterprise": {
     "badge": "Höchste Stufe"
    }
   }
  },
  "newsHub": {
   "streak": {
    "cardTitle": "Aktive Serie",
    "pageTitle": "Deine Serie",
    "pageDescription": "Bleib jeden Tag aktiv, um deine Serie auszubauen und Belohnungen freizuschalten.",
    "current": "Aktuell",
    "longest": "Längste",
    "totalDays": "Tage gesamt",
    "last30Days": "Letzte 30 Tage",
    "last90Days": "Letzte 90 Tage",
    "daysStreak": "Tage Serie",
    "days": "Tage",
    "viewDetails": "Details",
    "freezeTokens": "{{count}} Freeze",
    "freezeTokensLabel": "Freeze-Token",
    "nextMilestone": "{{remaining}} T bis {{next}}",
    "nextMilestoneLong": "Noch {{remaining}} Tage bis {{next}} Tage",
    "tooltipWithNext": "{{current}} Tage in Folge — noch {{remaining}} bis zur {{next}}-Tage-Belohnung",
    "tooltipMax": "{{current}} Tage in Folge — legendäre Serie!",
    "milestoneReached": "🔥 {{days}}-Tage-Serie erreicht!",
    "milestoneCredits": "+{{credits}} Bonus-Credits deinem Konto gutgeschrieben",
    "milestoneDollars": "+${{amount}} AI-Video-Guthaben deinem Konto gutgeschrieben 🎬",
    "milestoneDollarsShort": "+${{amount}} AI-Video-Guthaben",
    "milestonesTitle": "Meilensteine",
    "noMilestonesYet": "Noch keine Meilensteine — bleib dran!"
   },
   "upgrade": {
    "smart": {
     "credits": {
      "title": "Nur noch {count} Credits übrig",
      "body": "Deine Credits gehen zur Neige. Wechsle zu {plan} und hol dir jeden Monat ein frisches Paket, damit du nie mittendrin stoppen musst."
     },
     "feature": {
      "title": "{feature} ist eine {plan}-Funktion",
      "body": "Schalte {feature} und viele weitere Profi-Tools mit {plan} frei."
     },
     "streak": {
      "title": "🔥 {days}-Tage-Serie — du bist nicht zu stoppen!",
      "body": "Power-User wie du sparen mit {plan} am meisten. Mehr Credits, Premium-Funktionen und exklusive Vorteile."
     },
     "usage": {
      "title": "Du nutzt AdTool sehr intensiv",
      "body": "Nach deiner Aktivität bringt dir {plan} mehr Credits und schaltet genau die Funktionen frei, die du als Nächstes brauchst."
     },
     "manual": {
      "title": "Mehr freischalten mit {plan}",
      "body": "Wechsle zu {plan} und nutze den kompletten Profi-Funktionsumfang."
     },
     "annualSaving": "17 % pro Jahr sparen",
     "perfectForYou": "Perfekt für Power-User",
     "maybeLater": "Vielleicht später",
     "upgradeCta": "Jetzt upgraden",
     "benefit": {
      "credits": "{count} Credits pro Monat",
      "unlimited": "Unbegrenzte Credits",
      "posting": "Auto-Posting auf allen Plattformen",
      "team": "Team-Zusammenarbeit und Workspaces",
      "analytics": "Erweitertes Analytics-Dashboard",
      "brands": "Unbegrenzte Marken-Kits",
      "api": "Voller API-Zugriff",
      "priority": "Priorisierter Support"
     },
     "trial": {
      "halfwayTitle": "Die Hälfte deiner Testphase ist um — noch {days} Tage",
      "endingTitle": "Nur noch {days} Tage Enterprise",
      "lastDayTitle": "Letzter Tag deiner Enterprise-Testphase!",
      "graceTitle": "Testphase abgelaufen — Konto pausiert in {days} Tagen",
      "body": "Sichere dir jetzt {plan} und behalte alle Premium-Funktionen ohne Unterbrechung.",
      "lastDayBody": "Verliere morgen nicht den Zugriff — hol dir {plan} mit 20 % Rabatt im ersten Monat.",
      "graceBody": "Während der Kulanzfrist hast du weiterhin Zugriff. Wechsle jetzt zu {plan} und vermeide die Pause.",
      "couponLabel": "Dein Test-Rabatt",
      "couponDiscount": "20 % Rabatt im ersten Monat",
      "cta": "Mit TRIAL20 upgraden"
     },
     "discovery": {
      "title": "Du liebst {feature} — hol dir unbegrenzten Zugriff",
      "body": "Du hast {feature} schon mehrfach genutzt. {plan} schaltet unbegrenzte Renderings und priorisierte Verarbeitung frei."
     }
    }
   },
   "verifyEmail": {
    "loading": "Deine E-Mail wird geprüft …",
    "pleaseWait": "Bitte warten",
    "success": "Erfolgreich bestätigt! 🎉",
    "successDesc": "Deine E-Mail-Adresse ist bestätigt. Du hast jetzt vollen Zugriff auf alle Funktionen.",
    "error": "Bestätigung fehlgeschlagen",
    "noToken": "Kein Bestätigungs-Token gefunden",
    "expired": "Dieser Bestätigungslink ist ungültig oder abgelaufen. Fordere bitte eine neue E-Mail an.",
    "generic": "Die Bestätigung ist fehlgeschlagen. Bitte versuche es erneut.",
    "goDashboard": "Zum Dashboard",
    "resend": "Neue E-Mail senden",
    "backToLogin": "Zurück zur Anmeldung",
    "resentTitle": "Neue Bestätigungs-E-Mail gesendet!",
    "resentDesc": "Schau in dein Postfach",
    "resendError": "Senden fehlgeschlagen",
    "pleaseLogin": "Bitte melde dich erneut an"
   },
   "emailGate": {
    "badge": "E-Mail-Bestätigung erforderlich",
    "title": "Bestätige deine E-Mail",
    "subtitle": "Um AdTool AI vollständig zu nutzen, bestätige bitte deine E-Mail-Adresse.",
    "emailLabel": "E-Mail-Adresse",
    "instructions": "Wir haben dir eine Bestätigungs-E-Mail geschickt. Klicke auf den Link, um dein Konto zu aktivieren.",
    "checkSpam": "Schau auch in deinen Spam-Ordner",
    "resend": "Bestätigungs-E-Mail erneut senden",
    "resendIn": "Erneut senden in {seconds}s",
    "refresh": "Status aktualisieren",
    "sentTitle": "E-Mail gesendet!",
    "sentDesc": "Schau in dein Postfach und in den Spam-Ordner",
    "sendError": "Senden fehlgeschlagen",
    "tryAgain": "Bitte versuche es später erneut",
    "refreshError": "Aktualisieren fehlgeschlagen",
    "verified": "E-Mail bestätigt!",
    "welcome": "Willkommen bei AdTool AI!",
    "notYet": "Noch nicht bestätigt",
    "clickLink": "Klicke auf den Link in der E-Mail",
    "problemsContact": "Probleme? Kontaktiere unseren",
    "support": "Support"
   },
   "checkEmail": {
    "title": "Schau in dein Postfach",
    "subtitle": "Wir haben dir einen Bestätigungslink geschickt, um dein Konto zu aktivieren.",
    "sentTo": "E-Mail gesendet an:",
    "step1": "Öffne die E-Mail von AdTool AI",
    "step2": "Klicke auf den Bestätigungslink",
    "step3": "Denk daran, auch den Spam-Ordner zu prüfen",
    "resend": "E-Mail erneut senden",
    "resendIn": "Erneut senden in {seconds}s",
    "useDifferent": "Andere E-Mail verwenden",
    "resentTitle": "E-Mail erneut gesendet!",
    "resentDesc": "Schau in dein Postfach und in den Spam-Ordner",
    "resendError": "Erneutes Senden fehlgeschlagen",
    "notLoggedIn": "Melde dich erneut an, um die E-Mail zu senden",
    "problemsContact": "Brauchst du Hilfe? Kontaktiere unseren",
    "support": "Support"
   }
  }
 },
 "es": {
  "calendar": {
   "actions": {
    "addNoteLabel": "Añadir nota"
   },
   "messages": {
    "copySuccess": "Enlace copiado",
    "exportSuccess": "Exportación exitosa",
    "loadFailed": "No se pudieron cargar los eventos",
    "moveFailed": "Error al mover",
    "statusFailed": "Error al actualizar el estado",
    "statusUpdated": "Estado actualizado",
    "noEventsToExport": "No hay eventos para exportar",
    "pdfPrintDialog": "Abriendo el diálogo de impresión para exportar a PDF…",
    "saving": "Guardando…",
    "createEventComingSoon": "La creación de eventos estará disponible pronto",
    "addNoteComingSoon": "Añadir nota estará disponible pronto",
    "filterComingSoon": "La función de filtro estará disponible pronto",
    "shareComingSoon": "La función de compartir estará disponible pronto",
    "workspaceCreated": "Espacio de trabajo creado correctamente",
    "defaultWorkspace": "Mi espacio de trabajo"
   },
   "autoSchedule": {
    "description": "La IA sugiere las franjas horarias óptimas",
    "analyze": "Analizar",
    "applyAll": "Aceptar todo",
    "bestTime": "Mejor hora",
    "reason": "Motivo",
    "subtitle": "Deja que la IA encuentre las mejores horas para tu contenido",
    "selectEvents": "Seleccionar eventos",
    "noEvents": "Ningún evento seleccionado",
    "generate": "Generar programación",
    "generating": "Analizando las mejores horas…",
    "apply": "Aplicar sugerencias",
    "noSuggestions": "No hay sugerencias disponibles",
    "success": "Eventos programados correctamente",
    "error": "No se pudo generar la programación",
    "reasons": {
     "best_time": "Hora óptima de publicación",
     "high_engagement": "Franja de alta interacción",
     "low_competition": "Franja con poca competencia",
     "audience_active": "Audiencia más activa"
    }
   },
   "api": {
    "errors": {
     "MISSING_REQUIRED_FIELDS": "Faltan campos obligatorios",
     "TEMPLATE_NOT_FOUND": "Plantilla no encontrada",
     "NO_POSTS_IN_RANGE": "No hay publicaciones en el periodo",
     "POST_NOT_FOUND": "Publicación no encontrada",
     "SCHEDULE_CONFLICT": "Conflicto de programación",
     "GENERATION_FAILED": "Error de generación",
     "EXPORT_FAILED": "Error de exportación",
     "BLACKOUT_CONFLICT": "Esta hora entra en conflicto con una fecha bloqueada"
    },
    "success": {
     "POSTS_SCHEDULED": "{count} publicaciones programadas",
     "SCHEDULE_APPLIED": "Programación aplicada",
     "CAMPAIGN_CREATED": "Campaña con {count} eventos creada",
     "EVENT_RESCHEDULED": "Evento reprogramado",
     "EXPORT_READY": "Exportación lista",
     "scheduled": "Publicaciones programadas correctamente",
     "campaignGenerated": "Campaña generada correctamente",
     "blackoutAdded": "Fecha bloqueada añadida",
     "holidaysFetched": "Ideas de días festivos generadas"
    },
    "timeQuality": {
     "BEST_TIME": "Mejor hora",
     "high": "Franja horaria excelente",
     "medium": "Buena franja horaria",
     "low": "Franja horaria aceptable"
    }
   },
   "templates": {
    "title": "Plantillas de campaña",
    "subtitle": "Genera campañas a partir de plantillas predefinidas",
    "select": "Seleccionar plantilla",
    "campaignName": "Nombre de la campaña",
    "startDate": "Fecha de inicio",
    "generate": "Generar campaña",
    "generating": "Creando campaña…",
    "preview": "Vista previa",
    "duration": "Duración",
    "postsCount": "Publicaciones",
    "success": "Campaña generada correctamente",
    "error": "No se pudo generar la campaña",
    "types": {
     "launch": "Lanzamiento de producto",
     "sale": "Campaña de ventas",
     "season": "Contenido de temporada",
     "always_on": "Contenido siempre activo"
    }
   },
   "blackout": {
    "subtitle": "Bloquea las fechas en las que no se puede publicar",
    "startTime": "Hora de inicio",
    "endTime": "Hora de fin",
    "save": "Guardar",
    "success": "Fecha bloqueada añadida",
    "error": "No se pudo añadir la fecha bloqueada",
    "reasons": {
     "holiday": "Día festivo",
     "maintenance": "Mantenimiento del sistema",
     "event": "Evento especial",
     "other": "Otro"
    }
   },
   "copySuccess": "Enlace copiado al portapapeles",
   "exportSuccess": "Exportación exitosa",
   "loadFailed": "No se pudieron cargar los eventos",
   "moveFailed": "No se pudo mover el evento",
   "statusFailed": "No se pudo actualizar el estado",
   "statusUpdated": "Estado actualizado",
   "noEventsToExport": "No hay eventos para exportar",
   "pdfPrintDialog": "Abriendo el diálogo de impresión para exportar a PDF…",
   "saving": "Guardando…",
   "createEventComingSoon": "La creación de eventos estará disponible pronto",
   "addNoteComingSoon": "Añadir nota estará disponible pronto",
   "filterComingSoon": "La función de filtro estará disponible pronto",
   "shareComingSoon": "La función de compartir estará disponible pronto",
   "workspaceCreated": "Espacio de trabajo creado correctamente",
   "defaultWorkspace": "Mi espacio de trabajo",
   "apiDuplicate": {
    "errors": {
     "UNAUTHORIZED": "No autorizado",
     "MISSING_REQUIRED_FIELDS": "Faltan campos obligatorios",
     "TEMPLATE_NOT_FOUND": "Plantilla no encontrada",
     "NO_DRAFTS_AVAILABLE": "No hay borradores disponibles",
     "NO_POSTS_IN_RANGE": "No hay publicaciones en el rango",
     "POST_NOT_FOUND": "Publicación no encontrada",
     "SCHEDULE_CONFLICT": "Conflicto de programación",
     "CAPACITY_EXCEEDED": "Capacidad superada",
     "GENERATION_FAILED": "Error de generación",
     "EXPORT_FAILED": "Error de exportación",
     "INTERNAL_ERROR": "Error interno"
    },
    "success": {
     "POSTS_SCHEDULED": "{count} publicaciones programadas",
     "SCHEDULE_APPLIED": "Programación aplicada",
     "CAMPAIGN_CREATED": "Campaña creada con {count} eventos",
     "EVENT_RESCHEDULED": "Evento reprogramado",
     "EXPORT_READY": "Exportación lista"
    },
    "timeQuality": {
     "BEST_TIME": "Mejor hora",
     "GOOD_TIME": "Buena hora"
    }
   }
  },
  "pricingPage": {
   "plans": {
    "basic": {
     "description": "Perfecto para creadores de contenido y pequeñas empresas",
     "name": "Basic",
     "price": "14.99",
     "currency": "$",
     "period": "mes",
     "credits": "800 créditos",
     "features": [
      "Todas las plantillas premium",
      "Generador de hashtags",
      "Gestiona hasta 2 marcas",
      "Sin marca de agua",
      "Programación manual de publicaciones",
      "Soporte prioritario por correo",
      "Panel de analítica"
     ],
     "button": "Mejorar a Basic"
    },
    "pro": {
     "description": "Ideal para agencias y equipos",
     "name": "Pro",
     "price": "34.95",
     "currency": "$",
     "period": "mes",
     "credits": "2.500 créditos",
     "features": [
      "Todo lo de Basic",
      "Programación automática con IA",
      "Modelos de IA avanzados",
      "Herramientas de colaboración en equipo",
      "Panel de analítica",
      "Exportaciones de marca blanca",
      "Soporte prioritario y onboarding"
     ],
     "button": "Mejorar a Pro"
    },
    "enterprise": {
     "description": "Para grandes equipos y agencias",
     "name": "Enterprise",
     "price": "69.95",
     "currency": "$",
     "period": "mes",
     "credits": "Créditos ilimitados",
     "features": [
      "Todo lo de Pro",
      "Acceso a API e integraciones",
      "Soporte prioritario",
      "Herramientas de agencia y marca blanca",
      "Integraciones personalizadas",
      "Gestor de cuenta dedicado"
     ],
     "button": "Mejorar a Enterprise"
    }
   },
   "title": "Precios sencillos y transparentes",
   "subtitle": "Elige el plan que encaje con tu flujo de trabajo. Empieza gratis y mejora cuando quieras."
  },
  "heroBanner": {
   "title": "Planifica, publica y analiza publicaciones sociales – más rápido con IA.",
   "heading": "Planifica, publica y analiza publicaciones sociales: más rápido con IA.",
   "subheading": "Empieza gratis. Mejora cuando quieras.",
   "ctaPrimary": "Plan rápido",
   "ctaSecondary": "Abrir calendario",
   "stats": {
    "engagement": "Tasa de interacción ↑",
    "posts": "Publicaciones publicadas",
    "accounts": "Cuentas conectadas"
   },
   "trust": {
    "title": "Conforme al RGPD • Pago seguro",
    "subtitle": "Tus datos están seguros",
    "integrations": "Integraciones"
   }
  },
  "socialProof": {
   "title": "Lo que dicen nuestros usuarios",
   "testimonial1": "Con AdTool, ahorro 5 horas a la semana. ¡Las sugerencias de IA son oro!",
   "testimonial2": "Finalmente, una herramienta que entiende mi marca. La calidad del contenido es de primera.",
   "testimonial3": "Los conocimientos de rendimiento aumentaron mi engagement en un 30%."
  },
  "common": {
   "friendly": "Amigable",
   "professional": "Profesional",
   "funny": "Humorístico",
   "inspirational": "Inspirador",
   "bold": "Audaz",
   "emotional": "Emocional",
   "informative": "Informativo",
   "playful": "Juguetón",
   "close": "Cerrar",
   "language": "es",
   "error": "Error",
   "success": "Éxito",
   "warning": "Aviso",
   "metaDisconnected": "Conexión eliminada.",
   "metaGrantRemains": "Meta sigue mostrando esta app en tu perfil de Facebook. Abre Facebook → Configuración → Apps y sitios web y elimina «AdTool AI» para que la próxima conexión muestre el diálogo de permisos completo.",
   "cancel": "Cancelar",
   "generating": "Generando…",
   "uploading": "Subiendo…",
   "comingSoon": "Próximamente",
   "featureComingSoon": "¡Esta función estará disponible pronto!",
   "upgradeRequired": "Se requiere mejorar de plan",
   "upgradeToPro": "Mejorar a Pro",
   "locked": "Bloqueado",
   "requiresPro": "Requiere el plan Pro",
   "getStarted": "Empezar",
   "startNow": "Empieza ahora"
  },
  "calendar_title": "Calendario de Contenido Inteligente",
  "calendar_add_post": "Añadir Publicación",
  "calendar_add_note": "Añadir nota",
  "calendar_export": "Exportar a Google Calendar",
  "calendar_platform": "Plataforma",
  "calendar_caption": "Leyenda",
  "calendar_schedule_date": "Programar Fecha y Hora",
  "calendar_status": "Estado",
  "calendar_draft": "Borrador",
  "calendar_scheduled": "Programado",
  "calendar_posted": "Publicado",
  "calendar_note_text": "Nota",
  "calendar_upgrade_required": "Actualiza a Pro para crear y gestionar tu calendario de contenidos",
  "calendar_schedule_post": "Programar publicación",
  "calendar_image_upload": "Subir imagen (Opcional)",
  "calendar_tags": "Etiquetas (Opcional)",
  "trendRadar": {
   "trend_title": "Radar de Tendencias de IA",
   "fetch_button": "Recargar",
   "generate_post": "Generar contenido",
   "bookmark": "Guardar tendencia",
   "ideas": "Ideas de contenido",
   "trending_now": "Qué está en tendencia ahora",
   "filter_platform": "Plataforma",
   "filter_language": "Idioma",
   "filter_category": "Categoría",
   "popularity": "Popularidad",
   "view_ideas": "Ver ideas",
   "add_to_campaign": "Añadir a la campaña",
   "bookmarked": "Guardado",
   "analyzing": "Analizando tendencia...",
   "no_trends": "No se encontraron tendencias",
   "error_loading": "Error al cargar las tendencias"
  },
  "streak": {
   "cardTitle": "Racha activa",
   "pageTitle": "Tu racha",
   "pageDescription": "Mantente activo a diario para aumentar tu racha y desbloquear recompensas.",
   "current": "Actual",
   "longest": "Más larga",
   "totalDays": "Días totales",
   "last30Days": "Últimos 30 días",
   "last90Days": "Últimos 90 días",
   "daysStreak": "Días de racha",
   "days": "Días",
   "viewDetails": "Detalles",
   "freezeTokens": "{{count}} Congelación",
   "freezeTokensLabel": "Fichas de congelación",
   "nextMilestone": "{{remaining}}D hasta {{next}}",
   "nextMilestoneLong": "Faltan {{remaining}} días para la marca de {{next}} días",
   "tooltipWithNext": "{{current}} días seguidos — quedan {{remaining}} para la recompensa de {{next}} días",
   "tooltipMax": "{{current}} días seguidos — ¡racha legendaria!",
   "milestoneReached": "🔥 ¡Racha de {{days}} días alcanzada!",
   "milestoneCredits": "+{{credits}} créditos de bonificación acreditados en tu cuenta",
   "milestoneDollars": "+${{amount}} crédito de video con IA añadido 🎬",
   "milestoneDollarsShort": "+${{amount}} crédito de video con IA",
   "milestonesTitle": "Hitos",
   "noMilestonesYet": "Aún no hay hitos, ¡empieza ya!"
  },
  "upgrade": {
   "smart": {
    "credits": {
     "title": "Solo quedan {count} créditos",
     "body": "Te estás quedando sin créditos. Actualiza a {plan} para obtener un nuevo paquete de créditos mensuales y no volver a detenerte a mitad de camino."
    },
    "feature": {
     "title": "{feature} es una característica de {plan}",
     "body": "Desbloquea {feature} y muchas más herramientas Pro actualizando a {plan}."
    },
    "streak": {
     "title": "🔥 Racha de {days} días — ¡estás que ardes!",
     "body": "Los usuarios avanzados como tú ahorran más con {plan}. Más créditos, funciones premium y extras exclusivos."
    },
    "usage": {
     "title": "Estás usando AdTool de forma muy intensiva",
     "body": "Según tu actividad, {plan} te ofrece más créditos y desbloquea las funciones que necesitarás a continuación."
    },
    "manual": {
     "title": "Desbloquea más con {plan}",
     "body": "Actualiza a {plan} y utiliza el conjunto completo de funciones Pro."
    },
    "annualSaving": "Ahorra un 17% anualmente",
    "perfectForYou": "Perfecto para usuarios avanzados",
    "maybeLater": "Quizás más tarde",
    "upgradeCta": "Actualizar ahora",
    "benefit": {
     "credits": "{count} créditos al mes",
     "unlimited": "Créditos ilimitados",
     "posting": "Publicación automática en todas las plataformas",
     "team": "Colaboración en equipo y espacios de trabajo",
     "analytics": "Panel de análisis avanzado",
     "brands": "Kits de marca ilimitados",
     "api": "Acceso completo a la API",
     "priority": "Soporte prioritario"
    },
    "trial": {
     "halfwayTitle": "A mitad del período de prueba — quedan {days} días",
     "endingTitle": "Solo quedan {days} días de Enterprise",
     "lastDayTitle": "¡Último día de tu prueba Enterprise!",
     "graceTitle": "Prueba expirada — la cuenta se pausará en {days} días",
     "body": "Obtén {plan} ahora y mantén todas las funciones premium sin interrupciones.",
     "lastDayBody": "No pierdas el acceso mañana — obtén {plan} con un 20% de descuento el primer mes.",
     "graceBody": "Todavía tienes acceso durante el período de gracia. Actualiza a {plan} ahora y evita la pausa.",
     "couponLabel": "Tu descuento de prueba",
     "couponDiscount": "20% de descuento el primer mes",
     "cta": "Actualizar con TRIAL20"
    },
    "discovery": {
     "title": "Te encanta {feature} — obtén acceso ilimitado",
     "body": "Has usado {feature} varias veces. {plan} desbloquea renders ilimitados y procesamiento prioritario."
    }
   }
  },
  "verifyEmail": {
   "loading": "Verificando correo electrónico...",
   "pleaseWait": "Por favor, espera",
   "success": "¡Verificado con éxito! 🎉",
   "successDesc": "Tu dirección de correo electrónico ha sido confirmada. Ahora tienes acceso completo a todas las funciones.",
   "error": "Verificación fallida",
   "noToken": "No se encontró ningún token de verificación",
   "expired": "Este enlace de verificación no es válido o ha caducado. Por favor, solicita un nuevo correo electrónico.",
   "generic": "La verificación falló. Por favor, inténtalo de nuevo.",
   "goDashboard": "Al Panel de Control",
   "resend": "Enviar nuevo correo electrónico",
   "backToLogin": "Volver al inicio de sesión",
   "resentTitle": "¡Nuevo correo electrónico de verificación enviado!",
   "resentDesc": "Revisa tu bandeja de entrada",
   "resendError": "Error al enviar",
   "pleaseLogin": "Por favor, inicia sesión de nuevo"
  },
  "emailGate": {
   "badge": "Verificación de correo electrónico requerida",
   "title": "Verifica tu correo electrónico",
   "subtitle": "Para usar AdTool AI completamente, por favor confirma tu dirección de correo electrónico.",
   "emailLabel": "Dirección de correo electrónico",
   "instructions": "Te hemos enviado un correo electrónico de verificación. Haz clic en el enlace para activar tu cuenta.",
   "checkSpam": "También revisa tu carpeta de spam o correo no deseado",
   "resend": "Reenviar correo electrónico de verificación",
   "resendIn": "Reenviar en {seconds}s",
   "refresh": "Actualizar estado",
   "sentTitle": "¡Correo electrónico enviado!",
   "sentDesc": "Revisa tu bandeja de entrada y carpeta de spam",
   "sendError": "Error al enviar",
   "tryAgain": "Por favor, inténtalo de nuevo más tarde",
   "refreshError": "Error al actualizar",
   "verified": "¡Correo electrónico verificado!",
   "welcome": "¡Bienvenido a AdTool AI!",
   "notYet": "Aún no verificado",
   "clickLink": "Por favor, haz clic en el enlace del correo electrónico",
   "problemsContact": "¿Problemas? Contacta a nuestro",
   "support": "Soporte"
  },
  "checkEmail": {
   "title": "Revisa tus correos electrónicos",
   "subtitle": "Te hemos enviado un enlace de verificación para activar tu cuenta.",
   "sentTo": "Correo electrónico enviado a:",
   "step1": "Abre el correo electrónico de AdTool AI",
   "step2": "Haz clic en el enlace de verificación",
   "step3": "No olvides revisar tu carpeta de spam",
   "resend": "Reenviar correo electrónico",
   "resendIn": "Reenviar en {seconds}s",
   "useDifferent": "Usar una dirección de correo electrónico diferente",
   "resentTitle": "¡Correo electrónico enviado de nuevo!",
   "resentDesc": "Revisa tu bandeja de entrada y carpeta de spam",
   "resendError": "Envío fallido",
   "notLoggedIn": "Por favor, inicia sesión de nuevo para reenviar el correo electrónico",
   "problemsContact": "¿Necesitas ayuda? Contacta con nuestro",
   "support": "Soporte"
  },
  "analytics": {
   "empty": {
    "title": "Aún no hay datos de analítica",
    "body": "Sincroniza los últimos 30 días para ver tu rendimiento.",
    "cta": "Sincronizar ahora"
   }
  },
  "mediaLibrary": {
   "title": "Biblioteca de medios",
   "subtitle": "Gestiona todos tus recursos de contenido en un solo lugar",
   "upload": "Subir archivo",
   "uploadSuccess": "Archivo subido correctamente",
   "deleteSuccess": "Archivo eliminado correctamente",
   "searchPlaceholder": "Buscar archivos…",
   "fileType": "Tipo de archivo",
   "allTypes": "Todos los tipos",
   "images": "Imágenes",
   "videos": "Vídeos",
   "documents": "Documentos",
   "noMedia": "Aún no hay archivos multimedia",
   "uploadFirst": "Sube tu primer archivo para empezar"
  },
  "search": "Buscar",
  "email": "Correo electrónico",
  "error": "Error",
  "success": "Éxito",
  "cancel": "Cancelar",
  "hubs": {
   "einstellungen": "Ajustes"
  },
  "onb": {
   "accounts": "Conectar cuentas",
   "brandkit": "Configurar Brand Kit",
   "goal": "Definir objetivo",
   "plan": "Generar plan de 1 semana",
   "automation": "Activar publicación automática",
   "planGenerated": "¡Tu plan de 1 semana se ha creado!"
  },
  "heatmap": {
   "empty": {
    "cta": "Conectar cuentas"
   }
  },
  "pricing": {
   "quickPost": {
    "locked": {
     "title": "Programación automática bloqueada",
     "description": "Quick Post en el calendario está disponible a partir del plan Pro (2500 créditos/mes)."
    },
    "includedIn": "Incluido en"
   }
  }
 }
};
