...
BEGIN
  ...
  -- Zielstate, Guard-Mode und From-States sind Konstanten, nicht parametrisierbar
  ...
  IF NOT _res.applied THEN
    RETURN jsonb_build_object(
      'applied', false,
      'reason', CASE WHEN _res.reason = 'from_state_mismatch' THEN 'unexpected_state' ELSE _res.reason END,
      'state', _res.state
    );
  END IF;

  -- Legacy-Spiegel atomar mit dem kanonischen State
