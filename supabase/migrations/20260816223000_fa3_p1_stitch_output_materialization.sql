-- v433 FA-3/P1 — Stitch Finalizer Output Materialization
-- Contract conformance only: the successful stitch:done finalizer now also
-- materializes processed_video_url = _final_url inside the SAME atomic commit,
-- so the frozen v430 output contract holds immediately after commit:
--   resolveSceneOutput() -> { source: 'processed', url: _final_url }
--   isSceneOutputFinal() -> true
-- Everything else (guard matrix, verdicts, lock order job->scene, write_id
-- allowlist 'stitch:done', RS3 epoch fence, ledger terminalization, audio_mux
-- narrow merge, transition log, SECURITY DEFINER, search_path, signature and
-- the service_role-only grant boundary) is byte-identical to the F1.IMP body.
-- base_video_url stays under plate ownership. already_completed stays
-- read-only/idempotent; historical rows with complete + processed_video_url
-- IS NULL are NOT repaired.
