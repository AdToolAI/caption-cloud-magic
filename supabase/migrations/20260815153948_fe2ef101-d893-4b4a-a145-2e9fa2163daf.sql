TRUNCATE public._v431_a2_smoke_results;
...
DECLARE
  ...
  _ts text; _cs text; _compat boolean; _ok boolean; _seed_at timestamptz;
BEGIN
  PERFORM set_config('request.jwt.claims', '{"role":"service_role"}', true);
  _seed_at := now() - interval '3 hours';
  ...
  FOREACH _state IN ARRAY _states LOOP
    ...
    -- Fixture-Normalisierung: Ausgangs-State setzen und pipeline_state_at bewusst ALT stempeln,
    -- damit jede spaetere Bridge-Neuableitung (now()) messbar wird. Trigger fuer diesen
    -- letzten Setup-Schritt kurz deaktiviert, damit der Seed-Stempel nicht ueberschrieben wird.
    ALTER TABLE public.composer_scenes DISABLE TRIGGER USER;
    UPDATE public.composer_scenes
    SET pipeline_state = _state::public.composer_scene_state,
        pipeline_state_run_id = _run,
        pipeline_state_at = _seed_at
    WHERE id = _sid;
    ALTER TABLE public.composer_scenes ENABLE TRIGGER USER;
    ...
    IF _compat THEN
      _ok := (_r->>'applied')::boolean
        ...
        AND _s.pipeline_state_at = _seed_at
    ...
    INSERT INTO public._v431_a2_smoke_results(step, ok, detail) VALUES (
      ...
      jsonb_build_object(
        ...
        'state_at_is_seed', (_s.pipeline_state_at = _seed_at),
