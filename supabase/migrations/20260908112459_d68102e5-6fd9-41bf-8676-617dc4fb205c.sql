...
BEGIN
  ...
  -- 2. Idempotency first: same attempt + reason already credited → no-op.
  ...
  -- 3. Locate the matching charge (run-scoped first, scene/legacy second).
  ...
  -- 4. Bound: charge minus everything already refunded against that charge.
  ...
  -- 5. Idempotent credit via the canonical wallet function (unique refund_key index
  --    protects against concurrent callers).
  v_balance := public.refund_ai_video_credits(p_user_id, v_list, p_scene_id, v_key);

  SELECT id, amount_euros, (metadata ? 'refund_source') INTO v_refund_id, v_refunded, v_tagged
  FROM public.ai_video_transactions WHERE metadata->>'refund_key' = v_key LIMIT 1;

  -- Untagged row with our key = written by this call (a concurrent winner that
  -- went through this function would already be tagged; a raw legacy row was
  -- caught in step 2).
  IF v_refund_id IS NOT NULL AND NOT COALESCE(v_tagged, false) THEN
    ...
    WHERE id = v_refund_id AND user_id = p_user_id;
  ...
  -- Lost the race against a concurrent caller: exactly one row exists already.
  RETURN jsonb_build_object('outcome', 'already_refunded', 'amount_euros', 0,
