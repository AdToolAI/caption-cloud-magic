-- v431 G3.2.2-F1.IMP — Contract/Race/Merge tests (service_role execution)
-- Self-cleaning: removes all test rows after assertions.
DO $$
DECLARE
  ...
  _result jsonb;
BEGIN
  -- Setup minimal scene under an existing composer project.
  ...
  -- 3. Invalid write_id (attempt_no=2)
  ...
  INSERT INTO public.composer_pipeline_jobs (
    ...
    _other_job_id, _scene_id, _run_id, 431, 'audio_mux', 2,
  ...
  -- 6. dispatch_uncertain with matching external_job_id -> finalize (attempt_no=3)
  _other_job_id := gen_random_uuid();
  INSERT INTO public.composer_pipeline_jobs (
    id, scene_id, run_id, run_contract_version, stage, attempt_no,
    idempotency_key, status, external_job_id, provider, metadata,
    ...
    _other_job_id, _scene_id, _run_id, 431, 'audio_mux', 3,
    'f1-test-3', 'dispatch_uncertain', 'render-789', 'remotion',
    '{}'::jsonb, 2
  );
  ...
  -- 7. RS3 epoch-aware: pre-reset attempt rejected (attempt_no=4)
  _other_job_id := gen_random_uuid();
  ...
  INSERT INTO public.composer_pipeline_jobs (
    ...
    _other_job_id, _scene_id, _run_id, 431, 'audio_mux', 4,
    'f1-test-4', 'dispatched', 'render-rs3-old', 'remotion',
  ...
  -- 8. RS3 epoch-aware: post-reset attempt with matching reset_id -> finalize (attempt_no=5)
  _other_job_id := gen_random_uuid();
  INSERT INTO public.composer_pipeline_jobs (
    id, scene_id, run_id, run_contract_version, stage, attempt_no,
    idempotency_key, status, external_job_id, provider, metadata,
    ...
    _other_job_id, _scene_id, _run_id, 431, 'audio_mux', 5,
    'f1-test-5', 'dispatched', 'render-rs3-new', 'remotion',
    jsonb_build_object('rs3_reset_id', 'reset-epoch-1'), 2
  );

  _result := public.composer_finalize_lipsync_scene(
    _other_job_id, 'render-rs3-new', _scene_id,
    'https://example.com/final-rs3.mp4', 'stitch:done'
  );
  ...
  -- Cleanup test rows
  DELETE FROM public.composer_scene_transition_log WHERE scene_id = _scene_id;
  DELETE FROM public.composer_pipeline_jobs WHERE scene_id = _scene_id;
  DELETE FROM public.composer_scenes WHERE id = _scene_id;
