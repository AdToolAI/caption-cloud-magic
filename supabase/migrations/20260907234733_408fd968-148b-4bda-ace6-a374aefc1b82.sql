...
BEGIN
  ...
  -- Wallet-Lock zuerst: serialisiert konkurrente Refund-Events desselben Nutzers.
  ...
  IF EXISTS (
    ...
    WHERE user_id = v_purchase.user_id AND metadata->>'refund_key' = v_key
  ) THEN
  ...
  -- p_refund_amount_minor ist der KUMULATIVE Erstattungsbetrag dieser Zahlung.
  ...
  v_target_cum := round(v_granted * v_ratio, 2);

  -- Bereits für DIESE Zahlung angesetzter Rückbuchungsbetrag (kumulativ, nicht event-lokal).
  SELECT COALESCE(SUM(COALESCE((metadata->>'target_euros')::numeric, 0)), 0)
    INTO v_prev_target
  FROM public.ai_video_transactions
  WHERE user_id = v_purchase.user_id
    AND type = 'refund'
  ...
  v_target := round(v_target_cum - v_prev_target, 2);
  IF v_target <= 0 THEN
    RETURN jsonb_build_object(
      'reversed', false, 'reason', 'nothing_outstanding',
      'target_cumulative_euros', v_target_cum, 'already_targeted_euros', v_prev_target
    );
  END IF;
  ...
  INSERT INTO public.ai_video_transactions (
    ...
    jsonb_build_object(
      ...
      'target_cumulative_euros', v_target_cum,
      'previously_targeted_euros', v_prev_target,
      'refunded_amount_minor', p_refund_amount_minor,
      'charge_amount_minor', p_charge_amount_minor,
