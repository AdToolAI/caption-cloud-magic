CREATE OR REPLACE FUNCTION public.composer_finalize_plate_scene(_pipeline_job_id uuid, _external_job_id text, _write_id text, _base_url text, _clip_source_hint text DEFAULT NULL::text, _extra jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Compatibility path: the legacy->state bridge re-derives pipeline_state/substate from the
  -- legacy mirrors when the state columns are not part of the write. Restore the original
  -- canonical tuple; no forward or backward move is allowed here.
  IF _compat THEN
    SELECT pipeline_state, pipeline_substate, pipeline_state_run_id
      INTO _post_state, _post_substate, _post_run_id
      FROM public.composer_scenes WHERE id = _scene.id;

    IF _post_state IS DISTINCT FROM _scene.pipeline_state
       OR _post_substate IS DISTINCT FROM _scene.pipeline_substate
       OR _post_run_id IS DISTINCT FROM _scene.pipeline_state_run_id THEN
      UPDATE public.composer_scenes
      SET pipeline_state = _scene.pipeline_state,
          pipeline_substate = _scene.pipeline_substate,
          pipeline_state_run_id = _scene.pipeline_state_run_id,
          pipeline_state_at = _scene.pipeline_state_at,
          updated_at = now()
      WHERE id = _scene.id;
    END IF;

    -- Timestamp-only stabilization (v431 G3.2.1): every state-touching write above re-enters
    -- the bridge, which stamps pipeline_state_at = now(). A state-preserving compatibility
    -- finalize must not pretend the scene just entered plate_ready/audio_prep/audio_ready.
    -- This write touches no legacy column and no state/substate, so the bridge is a no-op.
    UPDATE public.composer_scenes
    SET pipeline_state_at = _scene.pipeline_state_at
    WHERE id = _scene.id
      AND pipeline_state_at IS DISTINCT FROM _scene.pipeline_state_at;
  END IF;
