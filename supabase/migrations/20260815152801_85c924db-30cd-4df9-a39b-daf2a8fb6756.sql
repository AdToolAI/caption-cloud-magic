-- Compatibility path must not touch lip-sync mirrors: the active legacy->state bridge would
-- otherwise re-derive pipeline_state from them (twoshot_stage='master_clip' => audio_ready).
...
DROP TABLE IF EXISTS public._v431_a_smoke_results;
CREATE TABLE public._v431_a_smoke_results (
  id bigserial PRIMARY KEY,
  step text NOT NULL,
  ok boolean,
  detail jsonb
);
REVOKE ALL ON TABLE public._v431_a_smoke_results FROM PUBLIC, anon, authenticated;
...
DECLARE
  ...
  _state text; _states text[] := ARRAY['plate_rendering','plate_ready','audio_prep','audio_ready'];
  _ord integer := 0;
...
BEGIN
  ...
  FOREACH _state IN ARRAY _states LOOP
    ...
    _ord := _ord + 1;
    INSERT INTO public.composer_scenes(project_id, order_index, active_run_id, plate_generation, pipeline_state, clip_status)
    VALUES (_pid, _ord, _run, 3, _state::public.composer_scene_state, 'generating') RETURNING id INTO _sid;
    ...
    VALUES (_sid, _run, 'base_video', 'replicate', 'v431a-'||gen_random_uuid()::text, 'dispatched', 3, 'pred_'||_state)
    RETURNING id INTO _job;

    SELECT to_jsonb(s) INTO _before FROM public.composer_scenes s WHERE id = _sid;
    _r := public.composer_finalize_plate_scene(_job, 'pred_'||_state, 'ccw:plate-complete', 'https://x/'||_state||'.mp4', NULL,
    ...
    INSERT INTO public._v431_a_smoke_results(step, ok, detail) VALUES (
      'allowed:'||_state,
      (_r->>'applied')::boolean
        AND _s.base_video_url = 'https://x/'||_state||'.mp4'
        AND _s.clip_url = 'https://x/'||_state||'.mp4'
        AND _s.clip_status = 'ready' AND _s.clip_error IS NULL
        AND _s.processed_video_url IS NULL
        AND _j.status = 'succeeded'
        AND (CASE WHEN _state = 'plate_rendering' THEN _s.pipeline_state::text = 'plate_ready'
                  ELSE _s.pipeline_state::text = _state
    ...
    _r := public.composer_finalize_plate_scene(_job, 'pred_'||_state, 'ccw:plate-complete', 'https://x/DUP.mp4', NULL, '{}'::jsonb);
    SELECT to_jsonb(s) INTO _after FROM public.composer_scenes s WHERE id = _sid;
  ...
  FOREACH _state IN ARRAY _rejected LOOP
    ...
    INSERT INTO public.composer_pipeline_jobs(scene_id, run_id, stage, provider, idempotency_key, status, plate_generation, external_job_id)
    VALUES (_sid, _run, 'base_video', 'replicate', 'v431r-'||gen_random_uuid()::text, 'dispatched', 3, 'pred_r_'||_state)
    RETURNING id INTO _job;

    SELECT to_jsonb(s) INTO _before FROM public.composer_scenes s WHERE id = _sid;
    SELECT to_jsonb(j) INTO _jb FROM public.composer_pipeline_jobs j WHERE id = _job;
    _r := public.composer_finalize_plate_scene(_job, 'pred_r_'||_state, 'ccw:plate-complete', 'https://x/new.mp4', NULL,
          jsonb_build_object('cinematic_sync', true));
    SELECT to_jsonb(s) INTO _after FROM public.composer_scenes s WHERE id = _sid;
    SELECT to_jsonb(j) INTO _ja FROM public.composer_pipeline_jobs j WHERE id = _job;
    SELECT to_jsonb(t) INTO _audit FROM public.composer_scene_transition_log t
