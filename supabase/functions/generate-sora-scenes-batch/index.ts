    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      while (!success && retryCount < 3) {
        try {
          const input: any = { 
            prompt: scene.prompt, 
            seconds: scene.duration || 8, 
            aspect_ratio: aspectRatio === '9:16' ? 'portrait' : aspectRatio === '1:1' ? 'square' : 'landscape',
            resolution: 'high'
          };
          if (scene.reference_image_url) input.input_reference = scene.reference_image_url;

          console.log(`🎬 Scene ${scene.scene_order}: Sending to Replicate with input =`, JSON.stringify(input));
          console.log(`🎬 Using version: ${SORA_VERSIONS[model as keyof typeof SORA_VERSIONS]}`);
          const prediction = await replicate.predictions.create({ version: SORA_VERSIONS[model as keyof typeof SORA_VERSIONS], input, webhook: webhookUrl, webhook_events_filter: ['completed', 'failed'] });
