...
serve(async (req) => {
  ...
  if (isQaMockRequest(req)) return qaMockJson(corsHeaders, { name: "structured-prompt-compose" });


  try {
    // Require authenticated user — this proxies a paid AI gateway, so we
    // must reject anonymous traffic to prevent credit abuse.
