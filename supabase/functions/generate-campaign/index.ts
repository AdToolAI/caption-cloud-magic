...
Language: ${language}`;
      ...
      // Queue fallback function for timeout handling
      const queueFallback = async () => {
        console.log('[Campaign] Timeout detected, enqueueing job');
        ...
        if (jobError) {
          console.error('[Campaign] Failed to enqueue job:', jobError);
          throw new Error('Failed to enqueue campaign generation');
        }

        console.log('[Campaign] Job enqueued:', job.id);
        return { queued: true, job_id: job.id };
      };

      // AI call with Circuit Breaker + Timeout + Queue Fallback
      const result: any = await withTimeoutOrQueue(
        aiCircuitBreaker.execute(async () => {
          console.log('[Campaign] Calling AI API');
          ...
          if (!response.ok) {
            ...
            console.error('[Campaign] AI API error:', response.status, errorText);
          ...
          if (!content) {
            console.error('[Campaign] No content in AI response');
            throw new Error('AI generation failed');
          }
          ...
          try {
            ...
            console.error('[Campaign] Failed to parse AI response:', parseError);
            throw new Error('Invalid AI response format');
          }

          // Validate structure
          if (!campaignPlan.weeks || !Array.isArray(campaignPlan.weeks)) {
            console.error('[Campaign] Invalid campaign structure:', campaignPlan);
            throw new Error('Invalid campaign structure generated');
          }
      ...
      if (result && typeof result === 'object' && 'queued' in result && result.queued) {
        return new Response(
          JSON.stringify({
            ...
            message: 'Campaign generation queued. Check job status for results.',
          }),
          { 
            status: 202,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
      ...
      if (postsError) {
        console.error('[Campaign] Error creating posts:', postsError);
      ...
      if (validation.data.media && validation.data.media.length > 0) {
        ...
        if (mediaError) {
          console.error('[Campaign] Error saving campaign media:', mediaError);
          // Continue anyway
        }
      }

      console.log('[Campaign] Successfully created campaign:', campaign.id);
      return new Response(
        JSON.stringify({
          campaign,
          plan: campaignPlan,
      ...
      console.error('[Campaign] Error in generate-campaign function:', error);
      return new Response(
