...
const V446_ANTI_PANEL_SUFFIX =
  "PANEL REJECTION RETRY — the previous attempt returned a SPLIT-SCREEN STRIP: the people were pasted side by side as separate panels with visible seams, mismatched backgrounds and different scales. That output is unusable. This is a TOPOLOGY correction only — keep the described action, poses, positions, props and setting EXACTLY as written (people may be driving, sitting, phoning, walking, working at a machine; do not move anyone into a different pose or location). Re-shoot it as ONE single photograph from ONE camera position inside ONE coherent physical world: one continuous background that runs unbroken from the left edge to the right edge of the image, NO vertical or horizontal dividing lines, NO seams, NO panel borders, NO collage tiles, NO stitched columns, NO video-call tiles, NO picture-in-picture. One shared perspective, one horizon line, one lighting direction and one depth of field for every person; people may overlap in depth as real people do, and the environment continues naturally behind all of them.";
