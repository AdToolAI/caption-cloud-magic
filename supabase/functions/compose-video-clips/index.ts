      ...
      try {
        ...
        if (wantsHeygen) {
          if (!hasDialog) {
            ...
            console.warn(
              `[compose-video-clips] Scene ${scene.id} wants HeyGen but no cast character — falling back to ${scene.clipSource}.`,
            );
          } else {
            ...
            const voiceCfgObj =
              typeof voiceCfg === "object" && voiceCfg
                ? (voiceCfg as DialogVoiceCfg)
                : undefined;
            const voiceId = resolveDialogVoiceId(voiceCfg);
            const voiceProvider = (voiceCfgObj?.provider || "")
              .toString()
              .toUpperCase();
            const voiceEngine = (voiceCfgObj?.engine || "")
              .toString()
              .toLowerCase();
            const isHumeVoice =
            ...
            const speakerCount = Math.max(
              1,
              countDialogSpeakers(scene.dialogScript),
            );

            if (!portraitUrl) {
              console.warn(
                `[compose-video-clips] Scene ${scene.id} HeyGen route — character ${primaryShot.characterId} has no portrait, falling back to ${scene.clipSource}.`,
              );
            } else if (!cleanText) {
              console.warn(
                `[compose-video-clips] Scene ${scene.id} HeyGen route — empty cleaned dialog, falling back to ${scene.clipSource}.`,
              );
            } else {
              if (speakerCount > 1) {
                console.warn(
                  `[compose-video-clips] Scene ${scene.id} HeyGen route: ${speakerCount} speakers detected — MVP uses first speaker only.`,
                );
              }
              ...
              if (isHumeVoice) {
                const humeVoiceName =
                  voiceCfgObj?.voiceName || voiceCfgObj?.voiceId;
                if (!humeVoiceName) {
                  ...
                  console.error(
                    `[compose-video-clips] Scene ${scene.id} Hume voice has no name`,
                    voiceCfgObj,
                  );
                  await supabaseAdmin
                    .from("composer_scenes")
                ...
                try {
                  const humeResp = await fetch(
                    `${supabaseUrl}/functions/v1/generate-voiceover-hume`,
                    {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json",
                  ...
                  if (!humeResp.ok) {
                    ...
                    throw new Error(
                      `Hume TTS ${humeResp.status}: ${errTxt.slice(0, 300)}`,
                    );
                  }
                  const humeData = await humeResp.json();
                  preSynthAudioUrl = humeData?.audioUrl;
                  if (!preSynthAudioUrl)
                    throw new Error("Hume response missing audioUrl");
                  console.log(
                    `[compose-video-clips] Scene ${scene.id} Hume TTS ok → ${preSynthAudioUrl.substring(0, 80)}…`,
                  );
                } catch (humeErr) {
                  ...
                  console.error(
                    `[compose-video-clips] Scene ${scene.id} Hume synth failed:`,
                    humeErr,
                  );
                  await supabaseAdmin
                    .from("composer_scenes")
                ...
                const looksLikeName =
                  /\s/.test(voiceId) || /[^A-Za-z0-9]/.test(voiceId);
                if (looksLikeName) {
                  ...
                  console.error(
                    `[compose-video-clips] Scene ${scene.id} suspicious voiceId:`,
                    voiceId,
                    voiceCfgObj,
                  );
                  await supabaseAdmin
              ...
              const heygenResp = await fetch(
                `${supabaseUrl}/functions/v1/generate-talking-head`,
                {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
              ...
              if (!heygenResp.ok) {
                ...
                console.error(
                  `[compose-video-clips] HeyGen scene ${scene.id} failed:`,
                  heygenResp.status,
                  errBody,
                );
                let userMsg = `HeyGen-Render fehlgeschlagen (${heygenResp.status}).`;
                if (
                  errBody.includes("invalid_uid") ||
                  errBody.includes("An invalid ID has been received")
                ) {
                ...
                console.log(
                  `[compose-video-clips] HeyGen scene ${scene.id} → video_id=${heygenData.predictionId}`,
                );

                // Deduct HeyGen credits (~€0.30 per speaker; MVP single speaker = €0.30).
                try {
                  const heygenCost =
                    0.3 * Math.max(1, Math.min(4, speakerCount));
                  ...
                  console.log(
                    `[compose-video-clips] Deducted €${heygenCost.toFixed(2)} for HeyGen scene ${scene.id}`,
                  );
                } catch (credErr) {
                  console.error(
                    "[compose-video-clips] HeyGen credit deduction failed:",
                    credErr,
                  );
                }
                ...
                results.push({
                  sceneId: scene.id,
                  status: "generating",
                  predictionId: heygenData.predictionId,
                });
              }
