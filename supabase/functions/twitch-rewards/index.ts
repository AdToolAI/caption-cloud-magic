...
Deno.serve(async (req) => {
  ...
  if (isQaMockRequest(req)) return qaMockJson(corsHeaders, { name: "twitch-rewards" });
  ...
  try {
    ...
    if (action === 'create') {
      const { title, cost, prompt, is_enabled } = body;
      response = await fetch(`${GATEWAY_URL}/channel_points/custom_rewards?broadcaster_id=${broadcaster_id}`, {
        method: 'POST',
        headers: {
      ...
      response = await fetch(`${GATEWAY_URL}/channels/vips?broadcaster_id=${broadcaster_id}&first=100`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${LOVABLE_API_KEY}`,
          'X-Connection-Api-Key': TWITCH_API_KEY,
    ...
    console.error('[twitch-rewards] Error:', error);
