...
function gen31Passes(): Array<{ idx: number; candidates: Candidate[] }> {
  type Pass = { idx: number; candidates: Candidate[] };
  const single = (idx: number, frame: number): Pass => ({ idx, candidates: [{ frame, validation: okVal, v530: { ok: true, decodeCompleted: true, decodeMs: 3, stillBytes: 100, stillDims: { width: 1284, height: 718 } } }] });
  const passes: Pass[] = [0, 1, 2, 3, 5].map((i, n) => single(i, 100 + n));
  passes.splice(4, 0, {
    idx: 4,
    candidates: [
