...
serve(async (req) => {
  ...
  if (isQaMockRequest(req)) return qaMockJson(corsHeaders, { name: "lambda-circuit-breaker" });
  ...
  try {
    ...
    const { data: configRows } = await supabase
      ...
      .in('key', [
        ...
        'lambda_circuit_breaker_min_sample',
      ]);

    const cfg: Record<string, unknown> = {};
    configRows?.forEach((r: any) => (cfg[r.key] = r.value));

    // Phase A defaults: NORMAL=25, SAFE=15, threshold=15%, min sample=10
    const SAFE = Number(cfg.lambda_max_concurrent_safe ?? 15);
    const NORMAL = Number(cfg.lambda_max_concurrent ?? 25);
    const THRESHOLD = Number(cfg.lambda_circuit_breaker_threshold ?? 0.15);
    const WINDOW_MIN = Number(cfg.lambda_circuit_breaker_window_min ?? 10);
    const MIN_SAMPLE = Number(cfg.lambda_circuit_breaker_min_sample ?? 10);
    const CURRENT = Number(cfg.lambda_max_concurrent ?? 25);
    ...
    // Need minimum sample size (10) to act — prevents false alarms
    if (total >= MIN_SAMPLE) {
