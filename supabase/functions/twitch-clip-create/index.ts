...
Deno.serve(async (req) => {
  ...
  if (isQaMockRequest(req)) return qaMockJson(corsHeaders, { name: "twitch-clip-create" });
  ...
  try {
    ...
    const response = await fetch(`${GATEWAY_URL}/clips?broadcaster_id=${broadcaster_id}`, {
      method: 'POST',
      headers: {
    ...
    console.error('[twitch-clip-create] Error:', error);
