...
serve(withTelemetry('generate-hooks', async (req) => {
  ...
  try {
    ...
    const requestSchema = z.object({
      ...
      styles: z.array(z.string().max(50)).min(1).max(10),
    ...
    const { topic, platform, tone, audience, styles, language } = validation.data;
