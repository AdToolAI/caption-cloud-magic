...
serve(async (req) => {
  ...
  try {
    ...
    const STRICT_RETRY_SUFFIX = strictMode
      ? (isMulti
        ? ` STRICT RETRY MODE — the previous attempt FAILED the cast-integrity audit (duplicated cast identity, swapped face, or missing cast member). Render each of the ${N} cast references EXACTLY ONCE — no duplicates, no twins, no mirror copies, no posters/screens of a cast person. Background extras are allowed but do not duplicate any cast face among them. Do NOT add a second instance of any cast member to balance the composition. V446 — the ${N} references are people to STAGE TOGETHER, never panels to arrange: the output is ONE photograph of ONE room with ONE unbroken background across the full image width. No strips, no columns, no tiles, no seams, no dividing lines, no collage, no stitched portraits, no video-call grid. Perspective, horizon, lighting direction and scale are shared by every person.`
