  ...
  try {
    ...
    const TWO_SHOT_FRAMING_SUFFIX = isMulti
      ? (gridRequested
        ...
        : (hasAsymmetricCast
          ? ` MULTI-CHARACTER FRAMING (asymmetric, per CHARACTER ACTIONS above): all ${N} cast people must be clearly visible and individually recognizable in the SAME frame. Screen share may be UNEQUAL — honor the per-character placement (foreground/background, primary/secondary, near/far) exactly as written in CHARACTER ACTIONS. Each cast face must still be unobstructed enough that a face detector can locate ${N} distinct cast faces (no full back-of-head, no fully hidden face, no silhouette). MANDATORY LIP-SYNC FRAMING: every named cast face MUST occupy AT LEAST 12 % of the frame width (roughly the size of a person filmed at medium distance). Do NOT compose a wide establishing shot where heads are tiny — actions like "on the phone", "printing", "typing" happen in a medium-distance ensemble, NOT in an architectural wide. Camera distance is medium at most.`
          : ` MANDATORY TWO-SHOT FRAMING: a wide ${N}-shot where ALL ${N} cast characters are fully visible in the SAME frame at roughly EQUAL screen share. Each cast face must be unobstructed, front-3/4 to camera, with clear separation between subjects. Every cast face MUST occupy AT LEAST 15 % of the frame width. NEVER produce a single-character close-up, NEVER cut a cast member out of frame, NEVER show only the back of a head, NEVER pull the camera so far back that heads become small in the frame.`))
      : "";
    ...
    const STRICT_RETRY_SUFFIX = strictMode
      ? (isMulti
        ? ` STRICT RETRY MODE — the previous attempt FAILED the cast-integrity audit (duplicated cast identity, swapped face, or missing cast member). Render each of the ${N} cast references EXACTLY ONCE — no duplicates, no twins, no mirror copies, no posters/screens of a cast person. Background extras are allowed but do not duplicate any cast face among them. Do NOT add a second instance of any cast member to balance the composition. V446/V453 — the ${N} references are people to STAGE TOGETHER, never panels to arrange: the output is ONE photograph of ONE room with ONE unbroken background across the full image width AND height. No strips, no columns, no rows, no 2x2 or 2x3 grid of tiles, no quadrants, no stacked portraits, no tiles, no seams, no dividing lines, no collage, no contact sheet, no stitched portraits, no video-call grid. Every person stands on the SAME ground plane in the SAME room at the SAME moment; perspective, horizon, lighting direction and scale are shared by every person.`
    ...
    const SINGLE_FRAME_SUFFIX = isMulti
      ? (gridRequested
        ...
        : ` SINGLE CONTINUOUS PHOTOGRAPH — the output is ONE unbroken photorealistic photograph taken with ONE camera in ONE moment. It is NOT a composite, NOT a grid, NOT a 2x2 layout, NOT a collage, NOT a stitched image, NOT a video-conference / Zoom / Teams / Meet screenshot, NOT a picture-in-picture, NOT a framed portrait wall. All ${N} people exist in ONE coherent physical world captured by ONE camera: SAME lighting direction, SAME perspective, SAME horizon line, SAME camera focal length — while keeping each person's described action, pose and position exactly as written (driving, sitting, phoning, working are all valid). Zero panel borders, zero dividing lines, zero separate frames.`)
      : "";
