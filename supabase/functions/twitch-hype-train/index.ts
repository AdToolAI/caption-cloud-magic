...
Deno.serve(async (req) => {
  ...
  if (isQaMockRequest(req)) return qaMockJson(corsHeaders, { name: "twitch-hype-train" });
  ...
  try {
    ...
    const response = await fetch(`${GATEWAY_URL}/hypetrain/events?broadcaster_id=${broadcaster_id}&first=1`, {
      method: 'GET',
      headers: {
    ...
    console.error('[twitch-hype-train] Error:', error);
