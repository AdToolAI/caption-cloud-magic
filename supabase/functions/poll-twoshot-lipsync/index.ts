...
function pickTargetCoordinates(
  ...
  mappingSource: "identity_match" | "pass_order";
  matchConfidence?: number;
  faceCenter?: [number, number];
  bbox?: [number, number, number, number];
  anchorDims?: { width: number; height: number };
  videoDims?: { width: number; height: number };
} {
  const charId = speakerContext?.characterId
    ? String(speakerContext.characterId).toLowerCase()
    : "";
  ...
  if (charId && faces.length) {
    const byId = faces.find(
      (f) => String(f.characterId ?? "").toLowerCase() === charId,
    );
    if (byId && byId.side) {
      match = byId;
      side = byId.side;
  ...
  if (!match) {
    match =
      faces.find((f) => f.side === side) ??
      faces[Math.min(passIndex, Math.max(0, faces.length - 1))];
  }
  if (match && Array.isArray(match.center) && match.center.length === 2) {
    ...
    const sameAspect =
      anchorW > 0 &&
      anchorH > 0 &&
      Math.abs(videoW / videoH - anchorW / anchorH) < 0.03;
    const scaleX = sameAspect ? videoW / anchorW : 1;
    const scaleY = sameAspect ? videoH / anchorH : 1;
    const bbox =
      Array.isArray(match.bbox) && match.bbox.length === 4
        ? match.bbox
        : undefined;
    const faceCenter: [number, number] = [
      Math.round(Number(match.center[0]) || 0),
      Math.round(Number(match.center[1]) || 0),
    ];
    let x = faceCenter[0];
    let y = faceCenter[1];
    if (bbox) {
    ...
    return {
      ...
      bbox,
      anchorDims:
        anchorW && anchorH ? { width: anchorW, height: anchorH } : undefined,
      videoDims: { width: videoW, height: videoH },
    };
  }
  ...
  return {
    coords: [
      Math.round(W * (side === "left" ? 0.3 : 0.7)),
      Math.round(H * 0.5),
    ],
    side,
    source: "heuristic",
...
async function startSyncJob(
  ...
  const normalizedSegments = (() => {
    ...
    const cleaned = arr
      .map(
        ([a, b]) =>
          [Math.max(0, Number(a)), Math.max(0, Number(b))] as [number, number],
      )
  ...
  const submit = async (withSegments: boolean) =>
    fetch("https://api.sync.so/v2/generate", {
      method: "POST",
  ...
  if (!resp.ok && useSegments) {
    ...
    if (
      /segments? configuration is invalid|invalid.+segment|only supported for video inputs/i.test(
        txt,
      ) ||
      resp.status === 400
    ) {
      console.warn(
        `[poll-twoshot-lipsync] segments_secs rejected, retrying without window: ${txt.slice(0, 200)}`,
      );
  ...
  if (!data?.id)
    throw new Error(
      `sync_create_missing_id: ${JSON.stringify(data).slice(0, 240)}`,
    );
...
async function pollSyncJob(
  ...
  const error =
    data?.error || data?.errorMessage || data?.message || data?.error_code;
  return {
    status,
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: scene, error: sErr } = await supabase
      ...
      .select(
        "id, project_id, clip_url, lip_sync_source_clip_url, audio_plan, lip_sync_status, twoshot_stage, lip_sync_applied_at, character_shots",
      )
      .eq("id", sceneId)
      .single();
    ...
    if (auth && userId !== project.user_id)
    ...
    const currentPass = isSegments
      ? 1
      : Number(syncJobs.currentPass || twoshot.heartbeat?.pass || 1);
    const totalPasses = isSegments
      ? 1
      : Number(syncJobs.totalPasses || jobs.length || 2);
    const fallbackMode = String(syncJobs.fallbackMode ?? "");
    const currentJob = isSegments
      ? (jobs[jobs.length - 1] ?? jobs[0])
      : (jobs.find((j: any) => Number(j?.pass) === currentPass) ??
        jobs[jobs.length - 1]);
    const jobId = String(
      currentJob?.jobId ||
        String(scene.replicate_prediction_id ?? "").replace(/^sync:/, ""),
    );
    ...
    if (
      ["PENDING", "PROCESSING", "RUNNING", "QUEUED"].includes(polled.status)
    ) {
      await supabase
        .from("composer_scenes")
        .update({
                ...
                jobs: jobs.map((j: any) =>
                  j.jobId === jobId
                    ? {
                        ...
                        lastPolledAt: now,
                        providerResponse: polled.providerResponse,
                      }
                    : j,
                ),
              },
    ...
    if (["FAILED", "REJECTED", "CANCELED"].includes(polled.status)) {
      const latest = await supabase
        .from("composer_scenes")
        .select("audio_plan, clip_url, lip_sync_source_clip_url")
        .eq("id", sceneId)
        .single();
      const latestPlan = (latest.data?.audio_plan ?? plan) as Record<
        string,
        any
      >;
      const latestTwoshot = (latestPlan.twoshot ?? {}) as Record<string, any>;
      const latestSyncJobs = (latestTwoshot.syncJobs ?? syncJobs) as Record<
        string,
        any
      >;
      const latestJobs = Array.isArray(latestSyncJobs.jobs)
        ? latestSyncJobs.jobs
        : jobs;
      const latestCurrentJob =
        latestJobs.find((j: any) => j.jobId === jobId) ?? currentJob;

      if (
        isSegments &&
        /segments configuration is invalid/i.test(String(polled.error ?? ""))
      ) {
      ...
      const TRANSIENT_REGEX =
        /(generation pipeline|internal|timeout|temporarily|rate.?limit|503|502|504)/i;
      const MAX_RETRIES = 2;
      ...
      if (
        ...
        try {
          await new Promise((r) =>
            setTimeout(r, retryAttempts === 0 ? 5_000 : 15_000),
          );
          const retrySegments = normalizeSegmentField(
            (latestCurrentJob as any).audioSegmentSecs,
          );
          const newJobId = await startSyncJob(syncApiKey, {
            ...
            targetCoords: Array.isArray(latestCurrentJob.targetCoords)
              ? (latestCurrentJob.targetCoords as [number, number])
              : null,
            faceBbox:
              Array.isArray(latestCurrentJob.faceBbox) &&
              latestCurrentJob.faceBbox.length === 4
                ? (latestCurrentJob.faceBbox as [
                    number,
                    number,
                    number,
                    number,
          ...
          console.warn(
            `[poll-twoshot-lipsync ${sceneId}] retry submit failed`,
      ...
      if (
        !fallbackTried &&
        !fallbackMode &&
        latestCurrentJob?.audioUrl &&
        latestCurrentJob?.videoUrl
      ) {
        try {
          ...
          const fallbackSegments = normalizeSegmentField(
            (latestCurrentJob as any).audioSegmentSecs,
          );
          const newJobId = await startSyncJob(syncApiKey, {
            videoUrl: String(latestCurrentJob.videoUrl),
          ...
          console.warn(
            `[poll-twoshot-lipsync ${sceneId}] isolated-track fallback submit failed`,
        ...
        if (cost > 0) {
          ...
          if (wallet) {
            await supabase
              .from("wallets")
              .update({
                balance: Number(wallet.balance ?? 0) + cost,
                updated_at: now,
              })
              .eq("user_id", userId);
          }
          ...
          audio_plan: {
            ...
            twoshot: {
              ...
              syncJobs: {
                ...
                lastError: polled.error ?? polled.status,
                lastErrorAt: now,
                jobs: latestJobs.map((j: any) =>
                  j.jobId === jobId
                    ? {
    ...
    if (polled.status !== "COMPLETED" || !polled.outputUrl) {
      return json(
        { error: "unexpected_sync_status", status: polled.status },
        502,
      );
    }
    ...
    if (!isSegments && currentPass < totalPasses) {
      ...
      const charShots = Array.isArray((scene as any).character_shots)
        ...
        : [];
      const target = pickTargetCoordinates(
        nextIdx,
        twoshot.faceMap as FaceMap | null,
        videoDims,
        {
          characterId: nextSpeaker.character_id ?? null,
          characterShots: charShots,
        },
      );
      if (
        !Number.isFinite(target.coords[0]) ||
        !Number.isFinite(target.coords[1]) ||
        target.coords[0] <= 0 ||
        target.coords[1] <= 0
      ) {
        return json(
          { error: "invalid_next_face_target", coords: target.coords },
          422,
        );
      }
        ...
        .update({
          ...
          audio_plan: {
            ...
            twoshot: {
              ...
              syncJobs: {
                ...syncJobs,
                mode: isTwoPass ? "two_pass" : syncJobs.mode,
                currentPass: nextPass,
                totalPasses,
                jobs: [...updatedJobs, nextJob],
              },
              heartbeat: {
                mode: isTwoPass ? "two_pass" : twoshot.heartbeat?.mode,
                pass: nextPass,
                total_passes: totalPasses,
                started_at: now,
                speaker: nextSpeaker.speaker,
    ...
    const publicUrl = await rehostVideo(
      supabase,
      userId!,
      sceneId,
      polled.outputUrl,
    ...
    const mergedSpeakers = prevSpeakers.map((sp: Record<string, unknown>) => ({
      ...sp,
      audioUrl: null,
      mergedInto: "twoshot",
    }));
    await supabase
      ...
      .update({
        ...
        lip_sync_source_clip_url:
          scene.lip_sync_source_clip_url || scene.clip_url,
